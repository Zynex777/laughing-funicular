export default async function handler(req,res){
  ...
  try{
    const r=await fetch("https://api.groq.com/openai/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${K}`},body:JSON.stringify({model:"llama-3.3-70b-versatile",max_tokens:500,messages:[{role:"system",content:"Especialista em marketing de afiliados. Responda em JSON válido."},{role:"user",content:`Produto:${productName},Preço:${price},Loja:${store},URL:${url}. Gere JSON: {"title":"...","description":"...","hashtags":"#tag1 #tag2 #tag3 #tag4 #tag5"}`}]})});
