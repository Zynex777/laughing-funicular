...
const ENV = (() => {
  try {
    return {
      ML_CLIENT_ID:    (typeof process !== "undefined" && process.env?.REACT_APP_ML_CLIENT_ID)    || "SEU_CLIENT_ID",
      ML_REDIRECT_URI: (typeof process !== "undefined" && process.env?.REACT_APP_ML_REDIRECT_URI) || (typeof window !== "undefined" ? (typeof window !== "undefined" ? window.location.origin : "https://turbo-pancake-omega.vercel.app") + "/" : "/"),
      ML_ACCESS_TOKEN: (typeof process !== "undefined" && process.env?.REACT_APP_ML_ACCESS_TOKEN) || null,
    };
  } catch(e) {
    return { ML_CLIENT_ID:"SEU_CLIENT_ID", ML_REDIRECT_URI:"/", ML_ACCESS_TOKEN:null };
  }
})();
    ...
    <div className="fu" style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <Card s={{ background:`linear-gradient(135deg,${C.neon}08,${C.blue}06)`, border:`1px solid ${C.neon}20` }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
          ...
          <Btn onClick={()=>goTo("Criar")} v="wa" s={{ padding:"10px 0", fontSize:12 }}>💬 Mensagem</Btn>
          <Btn onClick={()=>goTo("Criar")} v="u" s={{ padding:"10px 0", fontSize:12 }}>📝 Roteiro</Btn>
          <Btn onClick={()=>{ goTo("Postar"); }} v="u" s={{ padding:"10px 0", fontSize:12, background:`linear-gradient(135deg,#9b72f7,#4b8ef8)` }}>🤖 Auto-Fila</Btn>
        </div>
      </Card>

      <RegisterSaleCard stats={stats} updStats={updStats} links={links} />

      {/* ML Vídeo — indicador rápido */}
      {(videos||[]).some(v=>v.mlStatus) && (
        <Card s={{ background:"linear-gradient(135deg,#FFE60008,#FF990005)", border:"1px solid #FFE60020", padding:"12px 14px" }}>
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            <span style={{ fontSize:20 }}>🛒</span>
            <div style={{ flex:1 }}>
              <div style={{ color:"#FFE600", fontWeight:700, fontSize:12 }}>Vídeos ML</div>
              <div style={{ color:C.t3, fontSize:10 }}>
                Publicados: {(videos||[]).filter(v=>v.mlStatus==="success").length} · 
                Pendentes: {(videos||[]).filter(v=>v.mlStatus==="pending").length}
              </div>
            </div>
            <button onClick={()=>goTo("MLVideo")} style={{ background:"#FFE60020", border:"1px solid #FFE60040", borderRadius:8, padding:"5px 10px", color:"#FFE600", fontWeight:700, fontSize:10, cursor:"pointer" }}>Ver</button>
          </div>
        </Card>
      )}
      ...
      {result && (
        <Card glow={result.storeColor} s={{ border:`1px solid ${result.storeColor}45` }} className="fu">
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:8 }}>
            ...
            <Btn onClick={()=>{ saveLink(); setSelProd(result); goTo("Criar"); }} v="u" s={{ padding:"9px 0", fontSize:11 }}>📝 Roteiro</Btn>
            <Btn onClick={()=>{ saveLink(); setSelProd(result); goTo("Criar"); }} v="g" s={{ padding:"9px 0", fontSize:11 }}>🎬 Vídeo</Btn>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
          ...
          {scripts.map((s,i)=>(
            <div key={s.id} style={{ paddingBottom:i<scripts.length-1?14:0, marginBottom:i<scripts.length-1?14:0, borderBottom:i<scripts.length-1?`1px solid ${C.b1}`:"none" }}>
              ...
              <div style={{ display:"flex", gap:8, marginTop:8 }}>
                ...
                <button onClick={()=>{ const l=links.find(x=>x.productName===s.productName)||links[0]; if(l){setSelProd(l);goTo("Criar");} }} style={{ flex:1, background:C.gold+"18", border:`1px solid ${C.gold}35`, borderRadius:9, padding:"7px 0", color:C.gold, fontSize:11, fontWeight:700, cursor:"pointer" }}>🎬 Vídeo</button>
    ...
    try {
      ...
      if (data.movie) {
        ...
        const poll=setInterval(async()=>{
          ...
          try {
            const cr=await fetch(`/api/create-video?action=check?project=${data.movie}&apiKey=${encodeURIComponent(apiKey.trim())}`);
...
function Contas({ accs, setAccs, bestTimes, setBestTimes }) {
  ...
  const VALIDATORS = {
    tiktok:    { pattern: /^(@[\w.]+|https:\/\/(www\.)?tiktok\.com\/@[\w.]+)/i, hint: "Use @usuario ou https://tiktok.com/@usuario", prefix:"https://tiktok.com/" },
    instagram: { pattern: /^(@[\w.]+|https:\/\/(www\.)?instagram\.com\/[\w.]+)/i, hint: "Use @usuario ou https://instagram.com/usuario", prefix:"https://instagram.com/" },
    youtube:   { pattern: /^(https:\/\/(www\.)?youtube\.com\/@?[\w-]+|@[\w-]+)/i, hint: "Use https://youtube.com/@seucanal", prefix:"https://youtube.com/@" },
    facebook:  { pattern: /^(https:\/\/(www\.)?facebook\.com\/[\w./-]+|@[\w.]+)/i, hint: "Use https://facebook.com/suapagina", prefix:"https://facebook.com/" },
    kwai:      { pattern: /^(@[\w.]+|https:\/\/(www\.)?kwai\.com\/[\w.]+)/i, hint: "Use @usuario ou link do Kwai", prefix:"https://kwai.com/" },
    telegram:  { pattern: /^(https:\/\/t\.me\/[\w-]+|@[\w-]+|t\.me\/[\w-]+)/i, hint: "Use https://t.me/seucanal ou @canal", prefix:"https://t.me/" },
    twitter:   { pattern: /^(@[\w]+|https:\/\/(www\.)?twitter\.com\/[\w]+|https:\/\/x\.com\/[\w]+)/i, hint: "Use @usuario ou https://x.com/usuario", prefix:"https://x.com/" },
  };
  const [valErr, setValErr] = useState({});
  const save = (id) => {
    const val = inp[id]?.trim();
    if (!val) return;
    const validator = VALIDATORS[id];
    if (validator && !validator.pattern.test(val)) {
      setValErr(e => ({...e, [id]: validator.hint}));
      return;
    }
    setValErr(e => ({...e, [id]: null}));
    setAccs({...accs, [id]: val});
    setExp(null);
  };
                ...
                {open && (
                  <div style={{ padding:"0 12px 12px",borderTop:`1px solid ${C.b1}` }}>
                    ...
                    <input placeholder={`@sua_conta_${p.name.toLowerCase()}`} value={inp[p.id]||""} onChange={e=>setInp(i=>({...i,[p.id]:e.target.value}))}
                      style={{ width:"100%",background:C.card,border:`1px solid ${valErr[p.id]?C.red+"60":p.color+"40"}`,borderRadius:10,padding:"9px 12px",color:C.t1,fontSize:12,outline:"none",boxSizing:"border-box",marginBottom:valErr[p.id]?4:8 }}/>
                    {valErr[p.id] && <div style={{ color:C.red, fontSize:10, marginBottom:8, padding:"4px 8px", background:C.red+"10", borderRadius:6 }}>❌ {valErr[p.id]}</div>}
                    <div style={{ display:"flex",gap:8 }}>
...
function Config({ settings, setSetts, customStores, setCustomStores, connectedStores, setConnectedStores, mlTokens, setMlTokens, storeConfigs, setStoreConfigs, telegramBot, setTelegramBot, onLogout, onShowAdmin }) {
  ...
  return (
    <div className="fu" style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        {[{id:"contas",ico:"🏪",lbl:"Lojas"},{id:"tg",ico:"⚙️",lbl:"Contas"},{id:"app",ico:"📱",lbl:"App"}].map(t=>(
          <button key={t.id} onClick={()=>setConfigTab(t.id)}
            style={{ flex:1, padding:"9px 0", borderRadius:11,
              ...
              background:configTab===t.id?`${C.neon}15`:"transparent",
              color:configTab===t.id?C.neon:C.t3, fontWeight:700, fontSize:12, cursor:"pointer" }}>
            {t.ico} {t.lbl}
          </button>
        ))}
      </div>
                ...
                {(fields||[]).map(field=>(
                  <div key={field.key} style={{ marginBottom:10 }}>
                    ...
                    <div style={{ display:"flex",gap:8 }}>
                      ...
                      />
                      <button onClick={()=>{
                        const val = storeInputs[s.id+"_"+field.key]?.trim();
                        if (!val) return;
                        // Validate by field key and store
                        const VALIDATIONS = {
                          "amz_tag": { min:5, pattern:/^[a-zA-Z0-9][a-zA-Z0-9-]+-[0-9]{2}$/, msg:"Associate Tag inválido! Formato: seusite-20 (ex: meusite-20). Pegue em associados.amazon.com.br" },
                          "ml_token": { min:10, pattern:/^(APP_USR-|Bearer )?[A-Za-z0-9_-]{10,}/, msg:"Token ML inválido! Deve começar com APP_USR-. Pegue em developers.mercadolivre.com.br" },
                          "shp_id": { min:4, pattern:/^[A-Za-z0-9_-]{4,}$/, msg:"Affiliate ID da Shopee inválido! Mínimo 4 caracteres alfanuméricos." },
                          "shn_id": { min:4, pattern:/^[A-Za-z0-9_-]{4,}$/, msg:"Unique ID da Shein inválido! Mínimo 4 caracteres." },
                          "tmu_id": { min:4, pattern:/^[A-Za-z0-9_-]{4,}$/, msg:"Share ID da Temu inválido! Mínimo 4 caracteres." },
                          "nat_id": { min:4, pattern:/^[A-Za-z0-9_-]{4,}$/, msg:"ID da Natura inválido! Mínimo 4 caracteres." },
                          "bot_id": { min:4, pattern:/^[A-Za-z0-9_-]{4,}$/, msg:"ID do Boticário inválido! Mínimo 4 caracteres." },
                        };
                        const vkey = s.id+"_"+field.key;
                        const vRule = VALIDATIONS[vkey];
                        if (vRule) {
                          if (val.length < vRule.min || !vRule.pattern.test(val)) {
                            alert("❌ "+vRule.msg);
                            return;
                          }
                        } else if (val.length < 3) {
                          alert("❌ Valor muito curto! Mínimo 3 caracteres.");
                          return;
                        }
                        setStoreConfigs({...storeConfigs,[s.id]:{...cfg,[field.key]:val}});
        ...
        <div style={{ marginTop:14, paddingTop:14, borderTop:`1px solid ${C.b1}` }}>
          ...
          {mlTokens && mlTokens.access_token ? (
            ...
            <div>
              ...
              <button onClick={()=>{
                try {
                  const redirectUri = encodeURIComponent((typeof window !== "undefined" ? window.location.origin : "https://turbo-pancake-omega.vercel.app") + "/");
                  const mlClientId = (() => { try { return ENV.ML_CLIENT_ID; } catch(e) { return "SEU_CLIENT_ID"; } })();
      ...
      {/* ML OAuth section */}
      <Card s={{ background:"linear-gradient(135deg,#FFE60010,#FF990008)", border:"1px solid #FFE60030" }}>
        <div style={{ display:"flex", gap:12, alignItems:"center", marginBottom:12 }}>
          <div style={{ width:40, height:40, borderRadius:11, background:"linear-gradient(135deg,#FFE600,#FF9900)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0 }}>🛒</div>
          <div style={{ flex:1 }}>
            <div style={{ color:C.t1, fontWeight:700, fontSize:14 }}>Mercado Livre OAuth</div>
            <div style={{ color:C.t3, fontSize:11, marginTop:2 }}>Conecte sua conta para importar produtos e usar ML Vídeo</div>
          </div>
        </div>
        {mlTokens?.access_token || mlTokens?.token ? (
          <div>
            <div style={{ background:`${C.neon}10`, border:`1px solid ${C.neon}25`, borderRadius:10, padding:"9px 12px", marginBottom:10 }}>
              <div style={{ color:C.neon, fontSize:12, fontWeight:700 }}>✅ Conta conectada!</div>
              <div style={{ color:C.t3, fontSize:11, marginTop:2 }}>Token ativo — expira em 6h (limitação do ML)</div>
            </div>
            <button onClick={()=>setMlTokens({})}
              style={{ width:"100%", background:`${C.red}15`, border:`1px solid ${C.red}30`, borderRadius:10, padding:"9px 0", color:C.red, fontWeight:700, fontSize:12, cursor:"pointer" }}>
              🔌 Desconectar ML
            </button>
          </div>
        ) : (
          <div>
            <div style={{ background:`${C.gold}10`, border:`1px solid ${C.gold}25`, borderRadius:10, padding:"9px 12px", marginBottom:10 }}>
              <div style={{ color:C.gold, fontSize:11, lineHeight:1.8 }}>
                <strong style={{color:C.t1}}>Para conectar:</strong><br/>
                1. Configure ML_CLIENT_ID no Vercel<br/>
                2. Toque no botão abaixo<br/>
                3. Faça login com sua conta ML<br/>
                4. Autorize o app
              </div>
            </div>
            <a href={`https://auth.mercadolivre.com.br/authorization?response_type=code&client_id=${ENV.ML_CLIENT_ID}&redirect_uri=${encodeURIComponent(window.location.origin+"/")}`}
              style={{ display:"block", width:"100%", background:"linear-gradient(135deg,#FFE600,#FF9900)", border:"none", borderRadius:10, padding:"12px 0", color:"#000", fontWeight:800, fontSize:13, cursor:"pointer", textAlign:"center", textDecoration:"none", boxSizing:"border-box" }}>
      ...
      <Card>
        <div style={{ color:C.t1, fontWeight:700, fontSize:14, marginBottom:4 }}>✈️ Bot Telegram</div>
        <div style={{ color:C.t3, fontSize:11, marginBottom:12, lineHeight:1.7 }}>
          Configure seu bot para postar automaticamente no Telegram.
        </div>
        <div style={{ color:C.t2, fontSize:10, fontWeight:700, textTransform:"uppercase", marginBottom:4 }}>Token do Bot</div>
        <input
          value={telegramBot?.token || ""}
          onChange={e => setTelegramBot({...( telegramBot||{}), token:e.target.value})}
          placeholder="123456789:AABBcc..."
          style={{ width:"100%", background:C.s1, border:`1px solid ${C.b1}`, borderRadius:10, padding:"9px 12px", color:C.t1, fontSize:12, outline:"none", marginBottom:8, boxSizing:"border-box" }}
        />
        <div style={{ color:C.t2, fontSize:10, fontWeight:700, textTransform:"uppercase", marginBottom:4 }}>Chat ID do Canal</div>
        <input
          value={telegramBot?.chatId || ""}
          onChange={e => setTelegramBot({...(telegramBot||{}), chatId:e.target.value})}
          placeholder="-1001234567890"
          style={{ width:"100%", background:C.s1, border:`1px solid ${C.b1}`, borderRadius:10, padding:"9px 12px", color:C.t1, fontSize:12, outline:"none", marginBottom:10, boxSizing:"border-box" }}
        />
        <button onClick={()=>{
          if (!telegramBot?.token?.trim()) return alert("Token obrigatório!");
          if (!telegramBot?.chatId?.trim()) return alert("Chat ID obrigatório!");
          setTelegramBot({...telegramBot, active:true});
          alert("✅ Telegram configurado!");
        }}
          style={{ width:"100%", background:`linear-gradient(135deg,#2AABEE,#1a7db5)`, border:"none", borderRadius:11, padding:"11px 0", color:"#fff", fontWeight:700, fontSize:13, cursor:"pointer" }}>
          ✅ Salvar Configurações do Telegram
        </button>
        {telegramBot?.active && (
          <div style={{ marginTop:8, background:`${C.neon}10`, border:`1px solid ${C.neon}25`, borderRadius:9, padding:"8px 12px", color:C.neon, fontSize:11 }}>
            ✅ Telegram ativo e configurado!
          </div>
        )}
        <div style={{ marginTop:10, color:C.t3, fontSize:10, lineHeight:1.8 }}>
          Crie o bot em @BotFather · Pegue o Chat ID com @userinfobot
        </div>
      </Card>
          ...
          <div>
            <div style={{ background:`${C.gold}10`, border:`1px solid ${C.gold}25`, borderRadius:10, padding:"9px 12px", marginBottom:10 }}>
              <div style={{ color:C.gold, fontSize:11, lineHeight:1.8 }}>
                ...
                1. Configure ML_CLIENT_ID no Vercel<br/>
                2. Toque no botão abaixo<br/>
                3. Faça login com sua conta ML
              </div>
            </div>
            <a href={`https://auth.mercadolivre.com.br/authorization?response_type=code&client_id=${ENV.ML_CLIENT_ID}&redirect_uri=${encodeURIComponent(ENV.ML_REDIRECT_URI)}`}
              style={{ display:"block", width:"100%", background:"linear-gradient(135deg,#FFE600,#FF9900)", border:"none", borderRadius:10, padding:"12px 0", color:"#000", fontWeight:800, fontSize:13, cursor:"pointer", textAlign:"center", textDecoration:"none", boxSizing:"border-box" }}>
              🛒 Conectar com Mercado Livre
            </a>
          </div>
    ...
    <div className="fu" style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      {tab==="upload" && (
        <Card>
          ...
          <input type="file" accept="video/*" capture="environment" onChange={handleFile} id="vid-camera" style={{ display:"none" }}/>
          {!uploadFile ? (
            <div style={{ display:"flex", gap:10 }}>
              <label htmlFor="vid-upload" style={{ flex:1, display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:10,padding:"22px 10px",background:C.s1,border:`2px dashed ${C.b2}`,borderRadius:14,cursor:"pointer" }}>
                <div style={{ fontSize:36 }}>📹</div>
                <div style={{ color:C.t1,fontWeight:700,fontSize:12,textAlign:"center" }}>Selecionar da Galeria</div>
                <div style={{ color:C.t3,fontSize:10 }}>MP4, MOV, AVI</div>
              </label>
              <label htmlFor="vid-camera" style={{ flex:1, display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:10,padding:"22px 10px",background:`${C.blue}12`,border:`2px dashed ${C.blue}50`,borderRadius:14,cursor:"pointer" }}>
                <div style={{ fontSize:36 }}>📷</div>
                <div style={{ color:C.blue,fontWeight:700,fontSize:12,textAlign:"center" }}>Gravar Agora</div>
                <div style={{ color:C.t3,fontSize:10 }}>Abre câmera</div>
              </label>
            </div>
          ) : (
            <div>
  ...
  const createVideo = async () => {
    ...
    try {
      ...
      const poll = setInterval(async()=>{
        ...
        try {
          const cr = await fetch(`/api/create-video?action=check?project_id=${projectId}`);
          const st = await cr.json();
      ...
      {tab==="optimize" && (
        <Card>
          ...
          {links.length > 0 && (
            <div style={{ marginBottom:12 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                <div style={{ color:C.t2, fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:.8 }}>Produto</div>
                <button onClick={()=>setForm(f=>({
                  ...f,
                  linkId: f.linkId === "all" ? links[0]?.id || "" : "all"
                }))}
                  style={{ background:`${C.purple}15`, border:`1px solid ${C.purple}30`, borderRadius:8, padding:"3px 10px", color:C.purple, fontWeight:700, fontSize:10, cursor:"pointer" }}>
                  {form.linkId === "all" ? "❌ Desmarcar Todos" : "📦 Todos os Produtos"}
                </button>
              </div>
...
function buildOpenUrl(platform, text, link) {
  const url = link?.originalUrl || link?.trackedUrl || "";
  const enc = encodeURIComponent;
  const txt = (text || "").slice(0, 280);
  switch(platform) {
    // TikTok — opens upload page (text must be pasted manually)
    case "tiktok":
      return "https://www.tiktok.com/upload";
    // Instagram — opens create page
    case "instagram":
      return "https://www.instagram.com/create/story";
    // YouTube — opens studio upload
    case "youtube":
      return "https://studio.youtube.com";
    // Facebook — share dialog with text pre-filled
    case "facebook":
      return url
        ? `https://www.facebook.com/sharer/sharer.php?u=${enc(url)}&quote=${enc(txt)}`
        : "https://www.facebook.com/reel/create";
    // Kwai — opens upload
    case "kwai":
      return "https://www.kwai.com/creator/upload";
    // Telegram — share with text+link pre-filled ✅ works best
    case "telegram":
      return url
        ? `https://t.me/share/url?url=${enc(url)}&text=${enc(txt)}`
        : `https://t.me/share/url?url=${enc(txt)}`;
    // X/Twitter — tweet with text pre-filled
    case "twitter":
      return `https://twitter.com/intent/tweet?text=${enc(txt)}${url ? "&url="+enc(url) : ""}`;
    // WhatsApp — message pre-filled
    case "whatsapp":
      return `https://wa.me/?text=${enc(txt+(url?" "+url:""))}`;
    default:
      return "#";
  }
}
    ...
    <div className="fu" style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <>
          {pending.length === 0 ? (
            ...
            <Card>
              ...
              {/* ── INSTRUÇÃO DE POSTAGEM ── */}
              <div style={{ background:`${C.blue}10`, border:`1px solid ${C.blue}25`, borderRadius:12, padding:"10px 14px", marginBottom:8, display:"flex", gap:10, alignItems:"center" }}>
                <span style={{ fontSize:20 }}>📋</span>
                <div style={{ flex:1 }}>
                  <div style={{ color:C.blue, fontWeight:700, fontSize:12 }}>Como postar</div>
                  <div style={{ color:C.t3, fontSize:11, marginTop:2 }}>
                    O texto é <strong style={{color:C.t2}}>copiado automaticamente</strong>. Toque em Postar → o app abre → cole (pressione e segure) → publique.
                  </div>
                </div>
              </div>
              ...
              {pending.length > 0 && (
                <button onClick={()=>{
                  // Copy text of first pending item
                  const first = pending[0];
                  const txt = buildPostText(first.platform, first.link, first.script);
                  navigator.clipboard?.writeText(txt).catch(()=>{});
                  // Open all pending platforms one by one with small delay
                  pending.forEach((item, idx) => {
                    setTimeout(() => {
                      const url = buildOpenUrl(item.platform, buildPostText(item.platform, item.link, item.script), item.link);
                      window.open(url, "_blank");
                      // Mark as done
                      setQueue(q => q.map(qi => qi.id === item.id ? {...qi, status:"done", postedAt: new Date().toISOString()} : qi));
                    }, idx * 800);
                  });
                }}
                style={{ width:"100%", background:`linear-gradient(135deg,${C.neon},${C.blue})`, border:"none", borderRadius:14, padding:"14px 0", color:"#000", fontWeight:900, fontSize:15, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10, marginBottom:12, boxShadow:`0 0 20px ${C.neon}30` }}>
                  🚀 Postar Tudo ({pending.length} post{pending.length>1?"s":""}) de Uma Vez
                </button>
              )}
      ...
      {tab==="novo" && (
        <Card glow={C.neon}>
                  ...
                  <div key={l.id} onClick={()=>setForm(f=>({...f,linkId:l.id}))} style={{ display:"flex", gap:10, alignItems:"center", padding:"9px 12px", background:(form.linkId===l.id||(!form.linkId&&l===links[0]))?C.neon+"12":C.s1, border:`1px solid ${(form.linkId===l.id||(!form.linkId&&l===links[0]))?C.neon+"40":C.b1}`, borderRadius:11, cursor:"pointer" }}>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ color:C.t1, fontSize:12, fontWeight:600, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                        {l.productName && l.productName !== "Produto Mercado Livre" ? l.productName : l.originalUrl?.split("/").pop()?.slice(0,30) || l.productName || "Produto"}
                      </div>
                      <div style={{ display:"flex", gap:6, marginTop:1 }}>
                        {l.price && <span style={{ color:C.neon, fontSize:11 }}>{l.price}</span>}
                        {l.storeName && <span style={{ color:C.t3, fontSize:10 }}>{l.storeName}</span>}
                      </div>
                    </div>
          ...
          {/* Platforms */}
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
            <div style={{ color:C.t2, fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:.8 }}>Plataformas (pode selecionar várias)</div>
            <button onClick={()=>setForm(f=>({
              ...f,
              platforms: f.platforms.length === PLATFORMS.length ? [] : PLATFORMS.map(p=>p.id)
            }))}
              style={{ background:`${C.neon}15`, border:`1px solid ${C.neon}30`, borderRadius:8, padding:"3px 10px", color:C.neon, fontWeight:700, fontSize:10, cursor:"pointer" }}>
              {form.platforms.length === PLATFORMS.length ? "❌ Desmarcar Tudo" : "✅ Selecionar Tudo"}
            </button>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginBottom:14 }}>
            {PLATFORMS.map(p=>{
...
async function mlRefreshToken(refresh_token) {
  try {
    const res = await fetch("/api/ml-video?action=refresh", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function mlGetValidToken(tokens, setTokens) {
  if (!tokens) return ENV.ML_ACCESS_TOKEN;
...
function AutoPostPanel({ queue, setQueue, telegramBot, goTo }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      ...
      {pending.length === 0 && (
        <Card>
          <Empty ico="📋" title="Fila vazia" desc="Adicione posts na aba Fila usando a Auto-Fila da ARIA." action={<Btn onClick={() => goTo("Postar")} s={{ padding:"9px 18px" }}>📋 Ir para Fila</Btn>} />
        </Card>
      )}
...
// ══════════════════════════════════════════════════════
// ESPIÃO DE OFERTAS — Busca ofertas virais e replica
// ══════════════════════════════════════════════════════
function EspiaoOfertas({ addLink, goTo }) {
  const [loading, setLoading]   = useState(false);
  const [offers, setOffers]     = useState([]);
  const [store, setStore]       = useState("MLB");
  const [cat, setCat]           = useState("");
  const [added, setAdded]       = useState({});
  const [err, setErr]           = useState("");

  const STORES = [
    { id:"MLB", lbl:"Mercado Livre", ico:"🛒", color:"#FFE600" },
    { id:"AMZ", lbl:"Amazon",        ico:"📦", color:"#FF9900" },
    { id:"SHP", lbl:"Shopee",        ico:"🧡", color:"#EE4D2D" },
  ];

  const CATS = [
    { id:"",              lbl:"🔥 Mais Virais" },
    { id:"ELECTRONICS",  lbl:"📱 Eletrônicos" },
    { id:"FASHION",      lbl:"👗 Moda" },
    { id:"HOME",         lbl:"🏠 Casa" },
    { id:"SPORTS",       lbl:"⚽ Esportes" },
    { id:"BEAUTY",       lbl:"💄 Beleza" },
  ];

  const fetchOffers = async () => {
    setLoading(true); setErr(""); setOffers([]);
    try {
      // Mercado Livre API — busca produtos com maior desconto
      if (store === "MLB") {
        const catMap = {
          "ELECTRONICS": "MLB1051",
          "FASHION":     "MLB1430",
        ...
        const catId = catMap[cat] || "MLB1051";
        const url = `https://api.mercadolibre.com/sites/MLB/search?category=${catId}&sort=relevance&limit=20`;
        const res = await fetch(url);
        const data = await res.json();
        const items = (data.results || [])
          .filter(i => i.original_price && i.original_price > i.price)
          .sort((a,b) => {
            const da = Math.round((1 - a.price/a.original_price)*100);
            const db = Math.round((1 - b.price/b.original_price)*100);
            return db - da;
          })
          .slice(0, 12)
          .map(i => ({
            id: i.id,
            name: i.title,
            price: `R$ ${i.price.toFixed(2).replace('.',',')}`,
            original: `R$ ${i.original_price.toFixed(2).replace('.',',')}`,
            discount: Math.round((1 - i.price/i.original_price)*100),
            url: i.permalink,
            thumb: i.thumbnail?.replace("http://","https://"),
            store: "Mercado Livre",
            freeShip: i.shipping?.free_shipping,
            sold: i.sold_quantity || 0,
          }));
        setOffers(items);
        if (!items.length) setErr("Nenhuma oferta encontrada com desconto. Tente outra categoria.");
      } else if (store === "AMZ") {
        // Amazon — busca via URL pública (sem API key)
        setOffers([
          { id:"amz1", name:"Kindle Paperwhite 16GB", price:"R$ 479,00", original:"R$ 599,00", discount:20, url:"https://www.amazon.com.br/dp/B09TMF6742", store:"Amazon", freeShip:true, sold:0, thumb:"" },
          { id:"amz2", name:"Echo Dot 5ª Geração", price:"R$ 249,00", original:"R$ 349,00", discount:28, url:"https://www.amazon.com.br/dp/B09B8V1LZ3", store:"Amazon", freeShip:true, sold:0, thumb:"" },
          { id:"amz3", name:"Fone JBL Tune 510BT", price:"R$ 149,90", original:"R$ 199,90", discount:25, url:"https://www.amazon.com.br/dp/B08WM3NXQV", store:"Amazon", freeShip:false, sold:0, thumb:"" },
        ]);
        setErr("Amazon: mostrando ofertas populares. Para busca real configure API.");
      } else {
        setErr("Shopee: API requer configuração. Use ML por enquanto.");
      }
    } catch(e) {
      setErr("Erro ao buscar ofertas. Verifique sua conexão.");
    }
    setLoading(false);
  };

  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { fetchOffers(); }, [store, cat]);
  ...
  const addOffer = (o) => {
    ...
    const link = {
      ...
      price: o.price,
      storeName: o.store,
      storeColor: store==="MLB"?"#FFE600":store==="AMZ"?"#FF9900":"#EE4D2D",
      clicks: 0,
      commission: "0",
      createdAt: new Date().toISOString(),
    };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      ...
      {/* Header */}
      <Card glow={C.red} s={{ background:`linear-gradient(135deg,${C.red}10,${C.orange}08)`, border:`1px solid ${C.red}30` }}>
        <div style={{ display:"flex", gap:12, alignItems:"center" }}>
          <div style={{ width:50, height:50, borderRadius:14, background:`linear-gradient(135deg,${C.red},${C.orange})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:26, flexShrink:0 }}>🕵️</div>
          <div style={{ flex:1 }}>
            <div style={{ color:C.t1, fontWeight:900, fontSize:15 }}>Modo Espião</div>
            <div style={{ color:C.t3, fontSize:11, marginTop:2 }}>Ofertas mais virais agora — replique com 1 toque</div>
          </div>
          {loading && <Spin size={20} color={C.red} />}
        </div>
      </Card>
      ...
      <div style={{ display:"flex", gap:6 }}>
        {STORES.map(s => (
          <button key={s.id} onClick={()=>setStore(s.id)}
            style={{ flex:1, padding:"9px 0", borderRadius:12, border:`1px solid ${store===s.id?s.color+"60":C.b1}`, background:store===s.id?`${s.color}18`:"transparent", color:store===s.id?s.color:C.t3, fontWeight:700, fontSize:11, cursor:"pointer" }}>
            {s.ico} {s.lbl.split(" ")[0]}
          </button>
        ))}
      </div>
      ...
      <div style={{ display:"flex", gap:5, overflowX:"auto", scrollbarWidth:"none" }}>
        {CATS.map(ct => (
          <button key={ct.id} onClick={()=>setCat(ct.id)}
            style={{ flexShrink:0, padding:"6px 12px", borderRadius:99, border:`1px solid ${cat===ct.id?C.neon+"50":C.b1}`, background:cat===ct.id?`${C.neon}15`:"transparent", color:cat===ct.id?C.neon:C.t3, fontWeight:700, fontSize:10, cursor:"pointer", whiteSpace:"nowrap" }}>
            {ct.lbl}
          </button>
        ))}
      </div>
      ...
      {/* Error */}
      {err && (
        <div style={{ background:`${C.orange}10`, border:`1px solid ${C.orange}30`, borderRadius:12, padding:"10px 14px", color:C.orange, fontSize:11 }}>
          ⚠️ {err}
        </div>
      )}
      ...
      {loading && (
        <Card s={{ textAlign:"center", padding:32 }}>
          <Spin size={32} color={C.red} />
          <div style={{ color:C.t3, fontSize:13, marginTop:12 }}>Buscando ofertas virais...</div>
        </Card>
      )}

      {/* Offers list */}
      {!loading && (offers||[]).map(o => {
        const isAdded = added[o.id];
        return (
          <div key={o.id} style={{ background:C.card, border:`1px solid ${C.b1}`, borderRadius:16, overflow:"hidden" }}>
            <div style={{ padding:"12px 12px 0" }}>
              <div style={{ display:"flex", gap:10 }}>
                {/* Thumb */}
                <div style={{ width:64, height:64, borderRadius:10, background:C.s1, overflow:"hidden", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:28 }}>
                  {o.thumb ? <img src={o.thumb} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }}/> : "🛒"}
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ color:C.t1, fontWeight:600, fontSize:12, lineHeight:1.4, marginBottom:4 }}>{o.name}</div>
                  <div style={{ display:"flex", gap:6, alignItems:"center", flexWrap:"wrap" }}>
                    <span style={{ color:C.neon, fontWeight:900, fontSize:18 }}>{o.price}</span>
                    <span style={{ color:C.t3, fontSize:11, textDecoration:"line-through" }}>{o.original}</span>
                    <span style={{ background:C.red, color:"#fff", fontSize:10, fontWeight:800, padding:"2px 7px", borderRadius:6 }}>-{o.discount}%</span>
                  </div>
                  <div style={{ display:"flex", gap:5, marginTop:4, flexWrap:"wrap" }}>
                    {o.freeShip && <span style={{ background:`${C.neon}15`, color:C.neon, fontSize:9, fontWeight:700, padding:"2px 7px", borderRadius:6, border:`1px solid ${C.neon}30` }}>FRETE GRÁTIS</span>}
                    {o.sold > 0 && <span style={{ background:`${C.blue}15`, color:C.blue, fontSize:9, fontWeight:700, padding:"2px 7px", borderRadius:6 }}>🔥 {o.sold} vendidos</span>}
                  </div>
                </div>
              </div>
            </div>
            <div style={{ display:"flex", gap:0, marginTop:10, borderTop:`1px solid ${C.b1}` }}>
              <button onClick={()=>addOffer(o)}
                style={{ flex:2, background:isAdded?"transparent":`linear-gradient(135deg,${C.neon},${C.blue})`, border:"none", borderRight:`1px solid ${C.b1}`, padding:"11px 0", color:isAdded?C.neon:"#000", fontWeight:800, fontSize:12, cursor:"pointer" }}>
                {isAdded ? "✅ Adicionado!" : "➕ Adicionar Link"}
              </button>
              <button onClick={()=>window.open(o.url,"_blank")}
                style={{ flex:1, background:"none", border:"none", padding:"11px 0", color:C.t2, fontWeight:700, fontSize:12, cursor:"pointer" }}>
                🔍 Ver
              </button>
            </div>
          </div>
      ...
      {/* Empty */}
      {!loading && !err && !offers.length && (
        <Card s={{ textAlign:"center", padding:28 }}>
          <div style={{ fontSize:40, marginBottom:10 }}>🕵️</div>
          <div style={{ color:C.t1, fontWeight:700 }}>Nenhuma oferta encontrada</div>
          <div style={{ color:C.t3, fontSize:12, marginTop:6 }}>Tente outra categoria ou loja</div>
        </Card>
      )}
...
// ══════════════════════════════════════════════════════
// MONITOR TELEGRAM — Monitora canais de ofertas
// ══════════════════════════════════════════════════════
function TelegramMonitor({ telegramBot, addLink, queue, setQueue }) {
  const [channels, setChannels] = useState([
    { id:"1", name:"Promoções ML Brasil", url:"https://t.me/promocoesml", active:true },
    { id:"2", name:"Ofertas Amazon BR",   url:"https://t.me/ofertasamazonbr", active:false },
    { id:"3", name:"Shopee Promoções",    url:"https://t.me/shopeepromocoes", active:false },
  ]);
  const [newChan, setNewChan]   = useState("");
  const [monitor, setMonitor]   = useState(false);
  const [lastSeen, setLastSeen] = useState([]);
  const [interval_, setInt]     = useState(30);
  const [adding, setAdding]     = useState(false);

  const INTERVALS = [[10,"10 min"],[30,"30 min"],[60,"1 hora"],[120,"2 horas"]];

  const toggleChannel = (id) =>
    setChannels(ch => ch.map(c => c.id===id ? {...c, active:!c.active} : c));

  const addChannel = () => {
    if (!newChan.trim()) return;
    const url = newChan.startsWith("http") ? newChan : `https://t.me/${newChan.replace("@","")}`;
    const name = newChan.replace("https://t.me/","").replace("@","");
    setChannels(ch => [...ch, { id:Date.now().toString(), name, url, active:true }]);
    setNewChan("");
  };
  ...
  const checkNow = async () => {
    if (!telegramBot?.token) {
      alert("Configure o Bot Telegram na aba Config primeiro!");
      return;
    }
    setAdding(true);
    try {
      // Get updates from bot
      const res = await fetch(`https://api.telegram.org/bot${telegramBot.token}/getUpdates?limit=20`);
      const data = await res.json();
      const msgs = (data.result||[])
        .filter(u => u.channel_post?.text)
        .map(u => ({
          id: u.update_id.toString(),
          text: u.channel_post.text,
          chat: u.channel_post.chat?.title || "Canal",
          date: new Date(u.channel_post.date * 1000).toLocaleTimeString("pt-BR"),
        }));
      setLastSeen(msgs);
      if (!msgs.length) {
        setLastSeen([{ id:"0", text:"Nenhuma mensagem nova. Certifique que o bot está no canal como admin.", chat:"Info", date:"agora" }]);
      }
    } catch(e) {
      setLastSeen([{ id:"0", text:"Erro ao conectar: "+e.message, chat:"Erro", date:"agora" }]);
    }
    setAdding(false);
  };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      ...
      {/* Header */}
      <Card glow={C.tg} s={{ background:`linear-gradient(135deg,#2AABEE10,#1a7db510)`, border:`1px solid #2AABEE30` }}>
        <div style={{ display:"flex", gap:12, alignItems:"center" }}>
          <div style={{ width:50, height:50, borderRadius:14, background:"linear-gradient(135deg,#2AABEE,#1a7db5)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:26, flexShrink:0 }}>✈️</div>
          <div style={{ flex:1 }}>
            <div style={{ color:C.t1, fontWeight:900, fontSize:15 }}>Monitor Telegram</div>
            <div style={{ color:C.t3, fontSize:11, marginTop:2 }}>Monitora canais de ofertas e replica com seu link</div>
          </div>
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
            <div style={{ width:10, height:10, borderRadius:"50%", background:monitor?C.neon:C.t3, boxShadow:monitor?`0 0 8px ${C.neon}`:"none" }}/>
            <div style={{ color:monitor?C.neon:C.t3, fontSize:9, fontWeight:700 }}>{monitor?"ATIVO":"OFF"}</div>
          </div>
        </div>

        {/* Status */}
        {!telegramBot?.token && (
          <div style={{ marginTop:12, background:`${C.orange}15`, border:`1px solid ${C.orange}30`, borderRadius:10, padding:"9px 12px", color:C.orange, fontSize:11 }}>
            ⚠️ Configure o Bot Telegram na aba Config para usar o monitor.
          </div>
        )}
      </Card>
      ...
      {/* Interval + toggle */}
      <Card>
        <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:10 }}>⏱️ Verificar a cada</div>
        <div style={{ display:"flex", gap:6, marginBottom:12 }}>
          {INTERVALS.map(([v,l]) => (
            <button key={v} onClick={()=>setInt(v)}
              style={{ flex:1, padding:"8px 0", borderRadius:10, border:`1px solid ${interval_===v?C.neon+"50":C.b1}`, background:interval_===v?`${C.neon}15`:"transparent", color:interval_===v?C.neon:C.t3, fontWeight:700, fontSize:10, cursor:"pointer" }}>
              {l}
            </button>
          ))}
        </div>
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={()=>setMonitor(m=>!m)}
            style={{ flex:1, background:monitor?`${C.red}20`:`linear-gradient(135deg,${C.neon},${C.blue})`, border:monitor?`1px solid ${C.red}40`:"none", borderRadius:12, padding:"12px 0", color:monitor?C.red:"#000", fontWeight:800, fontSize:13, cursor:"pointer" }}>
            {monitor ? "⏹️ Parar Monitor" : "▶️ Iniciar Monitor"}
          </button>
          <button onClick={checkNow} disabled={adding}
            style={{ flex:1, background:C.s1, border:`1px solid ${C.b1}`, borderRadius:12, padding:"12px 0", color:adding?C.t3:C.neon, fontWeight:700, fontSize:12, cursor:"pointer" }}>
            {adding ? <Spin size={16} color={C.neon}/> : "🔄 Verificar Agora"}
          </button>
        </div>
      </Card>
      ...
      {/* Channels */}
      <Card>
        <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:12 }}>📡 Canais Monitorados</div>
        {channels.map(ch => (
          <div key={ch.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderBottom:`1px solid ${C.b1}` }}>
            <div style={{ width:36, height:36, borderRadius:10, background:"#2AABEE20", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>✈️</div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ color:C.t1, fontWeight:600, fontSize:12, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{ch.name}</div>
              <div style={{ color:C.t3, fontSize:10 }}>{ch.url}</div>
            </div>
            <div style={{ display:"flex", gap:6, flexShrink:0, alignItems:"center" }}>
              <div onClick={()=>toggleChannel(ch.id)}
                style={{ width:38, height:22, borderRadius:99, background:ch.active?C.neon:C.b1, cursor:"pointer", position:"relative", transition:"background .2s" }}>
                <div style={{ width:18, height:18, borderRadius:"50%", background:"#fff", position:"absolute", top:2, left:ch.active?18:2, transition:"left .2s" }}/>
              </div>
              <button onClick={()=>removeChannel(ch.id)}
                style={{ background:`${C.red}15`, border:`1px solid ${C.red}30`, borderRadius:8, padding:"5px 9px", color:C.red, fontWeight:700, fontSize:11, cursor:"pointer" }}>🗑</button>
            </div>
          </div>
        ))}
        {/* Add channel */}
        <div style={{ display:"flex", gap:8, marginTop:12 }}>
          <input value={newChan} onChange={e=>setNewChan(e.target.value)}
            placeholder="@canal ou t.me/canal"
            style={{ flex:1, background:C.s1, border:`1px solid ${C.b1}`, borderRadius:10, padding:"9px 12px", color:C.t1, fontSize:12, outline:"none" }}/>
          <button onClick={addChannel}
            style={{ background:`linear-gradient(135deg,${C.neon},${C.blue})`, border:"none", borderRadius:10, padding:"0 16px", color:"#000", fontWeight:800, fontSize:14, cursor:"pointer" }}>+</button>
        </div>
      </Card>
      ...
      {lastSeen.length > 0 && (
        <Card>
          <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:12 }}>📩 Últimas Mensagens</div>
          {lastSeen.map(m => (
            <div key={m.id} style={{ background:C.s1, borderRadius:12, padding:"10px 12px", marginBottom:8, border:`1px solid ${C.b1}` }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                <span style={{ color:"#2AABEE", fontSize:11, fontWeight:700 }}>✈️ {m.chat}</span>
                <span style={{ color:C.t3, fontSize:10 }}>{m.date}</span>
              </div>
              <div style={{ color:C.t2, fontSize:12, lineHeight:1.6 }}>{m.text.slice(0,120)}{m.text.length>120?"...":""}</div>
            </div>
          ))}
        </Card>
      )}
      ...
      {/* How it works */}
      <Card s={{ background:`${C.blue}08`, border:`1px solid ${C.blue}20` }}>
        <div style={{ color:C.blue, fontWeight:700, fontSize:13, marginBottom:10 }}>ℹ️ Como funciona</div>
        <div style={{ color:C.t3, fontSize:11, lineHeight:1.9 }}>
          1️⃣ Adicione seu bot como <strong style={{color:C.t2}}>admin</strong> nos canais<br/>
          2️⃣ Ative o monitor e defina o intervalo<br/>
          3️⃣ Quando detectar oferta, adicione ao seu link<br/>
          4️⃣ O bot posta automaticamente no seu canal
        </div>
      </Card>
...
// ══════════════════════════════════════════════════════
// HUB OFERTAS — Espião + Links + Monitor TG
// ══════════════════════════════════════════════════════

// ══════════════════════════════════════════════════════
// HUB CRIAR — Msgs + Roteiros + Vídeos + VideoAI + Meu Vídeo
// ══════════════════════════════════════════════════════

// ══════════════════════════════════════════════════════
// HUB POSTAR — AutoPost + Fila
// ══════════════════════════════════════════════════════

// ══════════════════════════════════════════════════════
// HUB MAIS — IA + Vitrine + Contas + Config
...
function HubOfertas({ addLink, goTo, telegramBot, queue, setQueue }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
      <div style={{ display:"flex", gap:6, marginBottom:14 }}>
        {[{id:"espia",ico:"🕵️",lbl:"Espião"},{id:"monitor",ico:"📡",lbl:"Monitor TG"}].map(s=>(
          <button key={s.id} onClick={()=>setSub(s.id)}
            style={{ flex:1, padding:"11px 0", borderRadius:13,
              border:`1px solid ${sub===s.id?C.red+"50":C.b1}`,
              background:sub===s.id?`linear-gradient(135deg,${C.red}18,${C.orange}10)`:"transparent",
              color:sub===s.id?C.red:C.t3, fontWeight:700, fontSize:12, cursor:"pointer" }}>
            {s.ico} {s.lbl}
          </button>
        ))}
      </div>
...
// ══════════════════════════════════════════════════════
// HUB CRIAR — Msgs + Roteiros + Vídeos + VideoAI + MeuVideo + AvatarAI
// ══════════════════════════════════════════════════════
function HubCriar({ links, videos, scripts, queue, setQueue, addScript, delScript,
                    selProd, setSelProd, addVideo, delVideo, goTo }) {
  const [sub, setSub] = useState("msgs");
  const SUBS = [
    { id:"msgs",    ico:"💬", lbl:"Mensagens" },
    { id:"rot",     ico:"📝", lbl:"Roteiros"  },
    { id:"vid",     ico:"🎬", lbl:"Vídeos"    },
    { id:"avatar",  ico:"🧑‍💻", lbl:"Avatar IA" },
    { id:"meuvid",  ico:"📹", lbl:"Meu Vídeo" },
    { id:"videoai", ico:"🤖", lbl:"VideoAI"   },
  ];
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
      <div style={{ display:"flex", gap:4, overflowX:"auto", scrollbarWidth:"none", marginBottom:14, paddingBottom:2 }}>
        {SUBS.map(s=>(
          <button key={s.id} onClick={()=>setSub(s.id)}
            style={{ flexShrink:0, padding:"7px 12px", borderRadius:99,
              border:`1px solid ${sub===s.id?C.purple+"50":C.b1}`,
              background:sub===s.id?`${C.purple}18`:"transparent",
              color:sub===s.id?C.purple:C.t3, fontWeight:700, fontSize:11, cursor:"pointer",
              display:"flex", alignItems:"center", gap:4 }}>
            <span>{s.ico}</span><span>{s.lbl}</span>
          </button>
        ))}
      </div>
      {sub==="msgs"    && <Mensagens links={links} />}
      {sub==="rot"     && <Roteiros links={links} selProd={selProd} setSelProd={setSelProd} scripts={scripts||[]} addScript={addScript} delScript={delScript} goTo={goTo} />}
      {sub==="vid"     && <Videos videos={videos||[]} links={links} selProd={selProd} setSelProd={setSelProd} goTo={goTo} addVideo={addVideo} delVideo={delVideo} />}
      {sub==="avatar"  && <AvatarVideo links={links} selProd={selProd} addVideo={addVideo} goTo={goTo} />}
      {sub==="meuvid"  && <MeuVideo links={links} selProd={selProd} setSelProd={setSelProd} addVideo={addVideo} queue={queue||[]} setQueue={setQueue} goTo={goTo} />}
      {sub==="videoai" && <VideoAICreator links={links} selProd={selProd} addVideo={addVideo} goTo={goTo} />}
    </div>
  );
}
...
// HUB POSTAR — AutoPost + Fila
// ══════════════════════════════════════════════════════
function HubPostar({ queue, setQueue, postLogs, setPostLogs, links, scripts,
                     telegramBot, setTelegramBot, goTo, videos }) {
  const [sub, setSub] = useState("autopost");
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
      <div style={{ display:"flex", gap:6, marginBottom:14 }}>
        {[{id:"autopost",ico:"🚀",lbl:"Auto-Post"},{id:"fila",ico:"📋",lbl:"Fila"}].map(s=>(
          <button key={s.id} onClick={()=>setSub(s.id)}
            style={{ flex:1, padding:"11px 0", borderRadius:13,
              border:`1px solid ${sub===s.id?C.gold+"50":C.b1}`,
              background:sub===s.id?`${C.gold}18`:"transparent",
              color:sub===s.id?C.gold:C.t3, fontWeight:700, fontSize:13, cursor:"pointer" }}>
            {s.ico} {s.lbl}
          </button>
        ))}
      </div>
      {sub==="autopost" && <AutoPostPanel queue={queue||[]} setQueue={setQueue} telegramBot={telegramBot||{}} goTo={goTo} />}
      {sub==="fila"     && <Fila queue={queue||[]} setQueue={setQueue} postLogs={postLogs||[]} setPostLogs={setPostLogs} links={links||[]} scripts={scripts||[]} telegramBot={telegramBot||{token:"",chatId:"",active:false}} setTelegramBot={setTelegramBot} />}
      {sub==="youtube"  && <YouTubePublisher videos={videos||[]} links={links||[]} queue={queue||[]} setQueue={setQueue} />}
    </div>
  );
}

// ══════════════════════════════════════════════════════
// HUB PERFIL — Vitrine + Contas
// ══════════════════════════════════════════════════════
function HubPerfil({ links, user, accs, setAccs, bestTimes, setBestTimes }) {
  const [sub, setSub] = useState("vitrine");
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
      <div style={{ display:"flex", gap:6, marginBottom:14 }}>
        {[{id:"vitrine",ico:"🏪",lbl:"Vitrine"},{id:"contas",ico:"📱",lbl:"Contas"}].map(s=>(
          <button key={s.id} onClick={()=>setSub(s.id)}
            style={{ flex:1, padding:"11px 0", borderRadius:13,
              border:`1px solid ${sub===s.id?C.neon+"50":C.b1}`,
              background:sub===s.id?`${C.neon}18`:"transparent",
              color:sub===s.id?C.neon:C.t3, fontWeight:700, fontSize:13, cursor:"pointer" }}>
            {s.ico} {s.lbl}
          </button>
        ))}
      </div>
      {sub==="vitrine" && <Vitrine links={links} user={user} />}
      {sub==="contas"  && <Contas accs={accs||{}} setAccs={setAccs} bestTimes={bestTimes||{}} setBestTimes={setBestTimes} />}
    </div>
  );
}

// ══════════════════════════════════════════════════════
// AVATAR VIDEO — Apresentador IA + Modo Gravação Própria
// ══════════════════════════════════════════════════════
function AvatarVideo({ links, selProd, addVideo, goTo }) {
  const [mode, setMode]       = useState("avatar");
  const [script_, setScript_] = useState("");
  ...
  const AVATARS = [
    { id:"ana",    name:"Ana",    desc:"Mulher jovem, animada", ico:"👩" },
    { id:"carlos", name:"Carlos", desc:"Homem adulto, sério",   ico:"👨" },
    { id:"julia",  name:"Julia",  desc:"Mulher profissional",   ico:"👩‍💼" },
    { id:"pedro",  name:"Pedro",  desc:"Homem jovem, casual",   ico:"🧑" },
  ];

  const prod = (links||[]).find(l=>l.id===selProd) || (links||[])[0];
  ...
  const genScript = async () => {
    ...
    try {
      const res = await fetch("/api/ai-manager", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({
          prompt: `Crie um roteiro de vídeo de 30 segundos para apresentar este produto de afiliado no TikTok/Instagram. Tom: animado e persuasivo. Produto: ${prod.productName}, Preço: ${prod.price}. Inclua: gancho de atenção nos primeiros 3 segundos, benefícios principais, CTA com urgência. Máximo 80 palavras. Sem hashtags.`,
          type: "script"
        })
      });
      const data = await res.json();
      setScript_(data.result || "Erro ao gerar roteiro.");
    } catch(e) { setScript_("Erro ao conectar com IA."); }
    setLoading(false);
  };

  const createAvatarVideo = async () => {
    if (!script_.trim()) { setErr("Escreva ou gere um roteiro primeiro!"); return; }
    ...
    try {
      const res = await fetch("/api/create-video", {
        ...
        body: JSON.stringify({
          ...
          avatar,
          lang,
          product: prod ? { name: prod.productName, price: prod.price, url: prod.trackedUrl||prod.originalUrl } : null,
        })
      });
      const data = await res.json();
      if (data.ok) {
        setResult(data);
        if (data.videoUrl) {
          addVideo && addVideo({ id:Date.now().toString(), url:data.videoUrl, productId:prod?.id, type:"avatar", createdAt:new Date().toISOString() });
        }
      } else {
        setErr(data.error || "Erro ao criar vídeo.");
      }
  ...
  const genCaption = async () => {
    ...
    try {
      const res = await fetch("/api/ai-manager", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({
          prompt: `Crie uma legenda viral para TikTok/Instagram para este vídeo de produto afiliado. Produto: ${prod?.productName||"produto"}. Inclua: emoji, CTA, 5 hashtags virais. Máximo 150 caracteres.`,
          type:"caption"
        })
      });
      const data = await res.json();
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      ...
      {/* Mode selector */}
      <div style={{ display:"flex", gap:6 }}>
        {[{id:"avatar",ico:"🧑‍💻",lbl:"Avatar IA"},{id:"proprio",ico:"📹",lbl:"Meu Vídeo"}].map(m=>(
          <button key={m.id} onClick={()=>setMode(m.id)}
            style={{ flex:1, padding:"11px 0", borderRadius:13,
              border:`1px solid ${mode===m.id?C.purple+"50":C.b1}`,
              background:mode===m.id?`linear-gradient(135deg,${C.purple}18,${C.blue}10)`:"transparent",
              color:mode===m.id?C.purple:C.t3, fontWeight:700, fontSize:12, cursor:"pointer" }}>
            {m.ico} {m.lbl}
          </button>
        ))}
      </div>
      ...
      {mode==="avatar" && (
        <>
          {/* Product */}
          <Card s={{ border:`1px solid ${C.purple}20` }}>
            <div style={{ color:C.t3, fontSize:10, fontWeight:700, marginBottom:6, textTransform:"uppercase" }}>Produto</div>
            {prod ? (
              <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                <div style={{ width:38, height:38, borderRadius:10, background:`${C.purple}15`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20 }}>🛒</div>
                <div style={{ flex:1 }}>
                  <div style={{ color:C.t1, fontWeight:600, fontSize:12 }}>{prod.productName}</div>
                  <div style={{ color:C.neon, fontWeight:700, fontSize:12 }}>{prod.price}</div>
                </div>
              </div>
            ) : (
              <button onClick={()=>goTo("Links")} style={{ width:"100%", background:`${C.purple}15`, border:`1px solid ${C.purple}30`, borderRadius:10, padding:"9px 0", color:C.purple, fontWeight:700, fontSize:12, cursor:"pointer" }}>
                🔗 Selecionar produto em Links
              </button>
            )}
          </Card>
          ...
          {/* Avatar selector */}
          <Card>
            <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:10 }}>🧑‍💻 Escolha o Apresentador</div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              {AVATARS.map(av=>(
                <button key={av.id} onClick={()=>setAvatar(av.id)}
                  style={{ padding:"10px 8px", borderRadius:12, border:`1px solid ${avatar===av.id?C.purple+"50":C.b1}`,
                    background:avatar===av.id?`${C.purple}18`:"transparent", cursor:"pointer", textAlign:"left" }}>
                  <div style={{ fontSize:24, marginBottom:4 }}>{av.ico}</div>
                  <div style={{ color:C.t1, fontWeight:700, fontSize:12 }}>{av.name}</div>
                  <div style={{ color:C.t3, fontSize:10 }}>{av.desc}</div>
                  {avatar===av.id && <div style={{ color:C.purple, fontSize:10, fontWeight:700, marginTop:3 }}>✅ Selecionado</div>}
                </button>
              ))}
            </div>
          </Card>
          ...
          <Card>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
              <div style={{ color:C.t1, fontWeight:700, fontSize:13 }}>📝 Roteiro do Vídeo</div>
              <button onClick={genScript} disabled={loading||!prod}
                style={{ background:`${C.purple}20`, border:`1px solid ${C.purple}40`, borderRadius:8, padding:"5px 12px", color:C.purple, fontWeight:700, fontSize:11, cursor:"pointer" }}>
                {loading?"⏳":"✨ Gerar com IA"}
              </button>
            </div>
            <textarea value={script_} onChange={e=>setScript_(e.target.value)}
              placeholder="Escreva o roteiro ou clique em ✨ Gerar com IA..."
              rows={5}
              style={{ width:"100%", background:C.s1, border:`1px solid ${C.b1}`, borderRadius:10, padding:"10px 12px", color:C.t1, fontSize:12, outline:"none", resize:"none", fontFamily:"inherit", lineHeight:1.7, boxSizing:"border-box" }}/>
            <div style={{ color:C.t3, fontSize:10, marginTop:5 }}>{script_.length} caracteres · máx 500</div>
          </Card>

          {/* Language */}
          <Card>
            <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:8 }}>🌐 Idioma</div>
            <div style={{ display:"flex", gap:6 }}>
              {[["pt-BR","🇧🇷 Português"],["en-US","🇺🇸 Inglês"],["es-ES","🇪🇸 Espanhol"]].map(([v,l])=>(
                <button key={v} onClick={()=>setLang(v)}
                  style={{ flex:1, padding:"8px 0", borderRadius:10, border:`1px solid ${lang===v?C.blue+"50":C.b1}`, background:lang===v?`${C.blue}15`:"transparent", color:lang===v?C.blue:C.t3, fontWeight:700, fontSize:10, cursor:"pointer" }}>
                  {l}
                </button>
              ))}
            </div>
          ...
          {/* Error */}
          {err && <div style={{ background:`${C.red}10`, border:`1px solid ${C.red}30`, borderRadius:10, padding:"9px 12px", color:C.red, fontSize:11 }}>❌ {err}</div>}

          {/* Result */}
          {result && (
            <Card glow={C.neon} s={{ background:`${C.neon}08` }}>
              <div style={{ color:C.neon, fontWeight:700, fontSize:13, marginBottom:8 }}>✅ Vídeo gerado!</div>
              {result.videoUrl && (
                <a href={result.videoUrl} target="_blank" rel="noreferrer"
                  style={{ display:"block", background:`linear-gradient(135deg,${C.neon},${C.blue})`, borderRadius:10, padding:"10px 0", color:"#000", fontWeight:800, fontSize:13, textAlign:"center", textDecoration:"none", marginBottom:8 }}>
                  ▶️ Ver Vídeo
                </a>
              )}
              {result.jobId && (
                <div style={{ color:C.t3, fontSize:11, textAlign:"center" }}>⏳ Processando... ID: {result.jobId}</div>
              )}
            </Card>
          )}

          {/* Create button */}
          <button onClick={createAvatarVideo} disabled={loading||!script_.trim()}
            style={{ width:"100%", background:loading||!script_.trim()?"#1a2540":`linear-gradient(135deg,${C.purple},${C.blue})`,
              border:"none", borderRadius:14, padding:"15px 0", color:loading||!script_.trim()?C.t3:"#fff",
              fontWeight:800, fontSize:14, cursor:loading||!script_.trim()?"default":"pointer",
              display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
            {loading ? <><Spin size={18} color="#fff"/> Gerando vídeo...</> : "🧑‍💻 Criar Vídeo com Avatar"}
          </button>

          <div style={{ background:`${C.blue}08`, border:`1px solid ${C.blue}20`, borderRadius:12, padding:"10px 14px" }}>
            <div style={{ color:C.blue, fontWeight:700, fontSize:11, marginBottom:4 }}>ℹ️ Como funciona</div>
            <div style={{ color:C.t3, fontSize:10, lineHeight:1.8 }}>
              Usa a API D-ID para gerar um apresentador virtual falando seu roteiro em português. Configure a chave DID_API_KEY no Vercel. Plano gratuito: 20 créditos/mês.
            </div>
          </div>
        </>
      )}

      {/* MODO B — Meu Vídeo com edição */}
      {mode==="proprio" && (
        <>
          <Card glow={C.blue} s={{ background:`${C.blue}08` }}>
            <div style={{ color:C.t1, fontWeight:700, fontSize:14, marginBottom:4 }}>📹 Cole o link do seu vídeo</div>
            <div style={{ color:C.t3, fontSize:11, marginBottom:12 }}>YouTube, Google Drive, Dropbox ou qualquer link direto</div>
            <input value={videoUrl} onChange={e=>setVideoUrl(e.target.value)}
              placeholder="https://youtube.com/... ou drive.google.com/..."
              style={{ width:"100%", background:C.s1, border:`1px solid ${C.b1}`, borderRadius:10, padding:"10px 12px", color:C.t1, fontSize:12, outline:"none", boxSizing:"border-box", marginBottom:10 }}/>
            <button onClick={genCaption} disabled={genCap||!videoUrl.trim()}
              style={{ width:"100%", background:`${C.blue}20`, border:`1px solid ${C.blue}40`, borderRadius:10, padding:"9px 0", color:C.blue, fontWeight:700, fontSize:12, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
              {genCap ? <><Spin size={14} color={C.blue}/> Gerando...</> : "✨ Gerar Legenda com IA"}
            </button>
          </Card>

          {caption && (
            <Card>
              <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:8 }}>💬 Legenda gerada</div>
              <div style={{ background:C.s1, borderRadius:10, padding:"10px 12px", color:C.t2, fontSize:12, lineHeight:1.7, marginBottom:10 }}>{caption}</div>
              <button onClick={()=>navigator.clipboard?.writeText(caption)}
                style={{ width:"100%", background:`${C.neon}15`, border:`1px solid ${C.neon}30`, borderRadius:10, padding:"9px 0", color:C.neon, fontWeight:700, fontSize:12, cursor:"pointer" }}>
                📋 Copiar Legenda
              </button>
            </Card>
          )}
          ...
          <Card>
            <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:10 }}>🎯 O que é adicionado automaticamente</div>
            {["✅ Legenda viral gerada por IA","✅ Link do produto rastreado na bio","✅ Hashtags virais do nicho","✅ CTA no final (ex: Link na bio!)","✅ Salvo na aba Vídeos"].map((item,i)=>(
              <div key={i} style={{ color:C.t2, fontSize:12, padding:"5px 0", borderBottom:i<4?`1px solid ${C.b1}`:"none" }}>{item}</div>
            ))}
          </Card>

          <button onClick={()=>{
            if (!videoUrl.trim()) return;
            addVideo && addVideo({ id:Date.now().toString(), url:videoUrl, caption, productId:prod?.id, type:"proprio", createdAt:new Date().toISOString() });
            setVideoUrl(""); setCaption("");
          }} disabled={!videoUrl.trim()}
            style={{ width:"100%", background:!videoUrl.trim()?"#1a2540":`linear-gradient(135deg,${C.blue},${C.purple})`,
              border:"none", borderRadius:14, padding:"15px 0", color:!videoUrl.trim()?C.t3:"#fff",
              fontWeight:800, fontSize:14, cursor:!videoUrl.trim()?"default":"pointer" }}>
...
// ══════════════════════════════════════════════════════
// MERCADO LIVRE VIDEO — Módulo de postagem de vídeos
// Adiciona vídeo ao anúncio via API oficial do ML
// ══════════════════════════════════════════════════════
function MercadoLivreVideo({ mlTokens, setMlTokens, videos, links }) {
  ...
  const [history, setHistory]     = useState(() => {
    try { return JSON.parse(localStorage.getItem("ml_video_history") || "[]"); } catch { return []; }
  });
  const [validation, setValidation] = useState(null);

  const token = (mlTokens||{})?.access_token || (mlTokens||{})?.token || "";

  // ── Save history ──
  const saveHistory = (entry) => {
    const updated = [{ ...entry, date: new Date().toISOString() }, ...history].slice(0, 50);
    setHistory(updated);
    localStorage.setItem("ml_video_history", JSON.stringify(updated));
  };

  // ── Call our backend ──
  const mlApi = async (action, data = {}) => {
    const res = await fetch(`/api/ml-video?action=${action}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const loadAccount = async () => {
    ...
    try {
      const data = await mlApi("account");
      if (data.ok) {
        setAccount(data.account);
        setSubView("items");
        loadItems(data.account.id);
      } else {
        setErr("Token inválido ou expirado. Reconecte o Mercado Livre em Config.");
      }
    } catch(e) { setErr("Erro ao conectar: " + e.message); }
    setLoading(false);
  };
  ...
  const loadItems = async (userId) => {
    setLoading(true);
    try {
      const data = await mlApi("items", { userId: userId || account?.id });
      if (data.ok) setItems(data.items || []);
      else setErr(data.error || "Erro ao carregar anúncios");
  ...
  const validateVideo = async (video) => {
    ...
    try {
      const data = await mlApi("validate", { videoUrl: video.url });
      setValidation(data);
    } catch(e) { setErr(e.message); }
    setLoading(false);
  };
  ...
  const uploadVideo = async () => {
    if (!selItem || !selVideo) return;
    setLoading(true); setErr(""); setJobStatus({ status: "uploading", progress: 0 });
    try {
      setJobStatus({ status: "uploading", progress: 30 });
      const data = await mlApi("upload", {
        videoUrl: selVideo.url,
        itemId:   selItem.id,
      });
      setJobStatus({ status: data.status || (data.ok ? "completed" : "failed"), progress: 100 });
      ...
      if (!data.ok && data.status === "unavailable") {
        setErr(data.message);
      } else if (data.ok) {
        setSubView("success");
      } else {
        setErr(data.error || "Erro ao enviar vídeo");
      }
    } catch(e) { setErr(e.message); setJobStatus({ status: "failed" }); }
    setLoading(false);
  };

  // ── ML connect URL ──
  const ML_AUTH_URL = `https://auth.mercadolivre.com.br/authorization?response_type=code&client_id=${ENV.ML_CLIENT_ID}&redirect_uri=${encodeURIComponent(ENV.ML_REDIRECT_URI)}`;

  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>

      {/* Header */}
      <Card glow="#FFE600" s={{ background:"linear-gradient(135deg,#FFE60010,#FF990008)", border:"1px solid #FFE60030" }}>
        <div style={{ display:"flex", gap:12, alignItems:"center" }}>
          <div style={{ width:50, height:50, borderRadius:14, background:"linear-gradient(135deg,#FFE600,#FF9900)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:26, flexShrink:0 }}>🛒</div>
          <div style={{ flex:1 }}>
            <div style={{ color:C.t1, fontWeight:900, fontSize:15 }}>Vídeo no Mercado Livre</div>
            <div style={{ color:C.t3, fontSize:11, marginTop:2 }}>Associe vídeos aos seus anúncios via API oficial</div>
          </div>
          {account && <Chip c={C.neon}>✅ {account.name}</Chip>}
        </div>
        ...
        <div style={{ display:"flex", gap:5, marginTop:14 }}>
          {[
            { id:"home",    lbl:"🏠 Início"    },
            { id:"items",   lbl:"📦 Anúncios"  },
            { id:"history", lbl:"📋 Histórico" },
          ].map(s => (
            <button key={s.id} onClick={()=>setSubView(s.id)}
              style={{ flex:1, padding:"7px 0", borderRadius:10,
                border:`1px solid ${subView===s.id?"#FFE60050":C.b1}`,
                background:subView===s.id?"#FFE60018":"transparent",
                color:subView===s.id?"#FFE600":C.t3, fontWeight:700, fontSize:10, cursor:"pointer" }}>
              {s.lbl}
            </button>
          ))}
        </div>
      ...
      {/* Error */}
      {err && (
        <div style={{ background:`${C.red}10`, border:`1px solid ${C.red}30`, borderRadius:12, padding:"11px 14px" }}>
          <div style={{ color:C.red, fontWeight:700, fontSize:13, marginBottom:4 }}>❌ Erro</div>
          <div style={{ color:C.t2, fontSize:12 }}>{err}</div>
          <button onClick={()=>setErr("")} style={{ marginTop:8, background:"transparent", border:"none", color:C.t3, fontSize:11, cursor:"pointer" }}>Fechar ×</button>
        </div>
      )}
      ...
      {subView === "home" && (
        <>
          {!token ? (
            <Card s={{ textAlign:"center", padding:"28px 16px" }}>
              <div style={{ fontSize:44, marginBottom:12 }}>🔐</div>
              <div style={{ color:C.t1, fontWeight:800, fontSize:15, marginBottom:8 }}>Conecte sua conta ML</div>
              <div style={{ color:C.t3, fontSize:12, lineHeight:1.8, marginBottom:16 }}>
                Configure ML_CLIENT_ID e ML_REDIRECT_URI no Vercel e conecte sua conta na aba <strong style={{color:C.t1}}>Config</strong>.
              </div>
              <div style={{ background:`${C.gold}10`, border:`1px solid ${C.gold}25`, borderRadius:11, padding:"10px 14px", marginBottom:16 }}>
                <div style={{ color:C.gold, fontSize:11, lineHeight:1.8 }}>
                  1. Vá em <strong>Config → Mercado Livre</strong><br/>
                  2. Clique em <strong>Conectar com ML</strong><br/>
                  3. Volte aqui depois de conectar
                </div>
              </div>
            </Card>
          ) : !account ? (
            <Card>
              <div style={{ color:C.t1, fontWeight:700, fontSize:14, marginBottom:12 }}>🔗 Conta conectada — Carregar dados</div>
              <button onClick={loadAccount} disabled={loading}
                style={{ width:"100%", background:`linear-gradient(135deg,#FFE600,#FF9900)`, border:"none", borderRadius:13, padding:"14px 0", color:"#000", fontWeight:800, fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
                {loading ? <><Spin size={18} color="#000"/> Carregando...</> : "🛒 Carregar Meus Anúncios"}
              </button>
            </Card>
          ) : (
            <Card>
              <div style={{ display:"flex", gap:12, alignItems:"center" }}>
                <div style={{ width:44, height:44, borderRadius:12, background:"#FFE60020", display:"flex", alignItems:"center", justifyContent:"center", fontSize:22 }}>👤</div>
                <div style={{ flex:1 }}>
                  <div style={{ color:C.t1, fontWeight:700, fontSize:14 }}>{account.name}</div>
                  <div style={{ color:C.neon, fontSize:11, marginTop:2 }}>✅ Mercado Livre conectado</div>
                </div>
              </div>
              <button onClick={()=>setSubView("items")}
                style={{ width:"100%", background:`linear-gradient(135deg,#FFE600,#FF9900)`, border:"none", borderRadius:13, padding:"13px 0", color:"#000", fontWeight:800, fontSize:13, cursor:"pointer", marginTop:14 }}>
                📦 Ver Meus Anúncios ({items.length})
              </button>
            </Card>
          )}
          ...
          <Card s={{ background:`${C.blue}08`, border:`1px solid ${C.blue}20` }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {[
                "1️⃣ Conecte sua conta Mercado Livre",
                "2️⃣ Selecione um anúncio ativo",
                "3️⃣ Escolha um vídeo cadastrado no app",
                "4️⃣ Valide os requisitos do ML",
                "5️⃣ Envie via API oficial do ML",
              ].map((step, i) => (
                <div key={i} style={{ color:C.t2, fontSize:12, padding:"5px 0", borderBottom:i<4?`1px solid ${C.b1}`:"none" }}>{step}</div>
              ))}
            </div>
            <div style={{ marginTop:12, background:`${C.orange}10`, border:`1px solid ${C.orange}25`, borderRadius:10, padding:"9px 12px", color:C.orange, fontSize:11 }}>
              ⚠️ Esta função requer aprovação especial do ML para sua conta. Se não disponível, o sistema informará claramente.
            </div>
          </Card>
        </>
      )}
      ...
      {subView === "items" && (
        <>
          {/* Video + Item selector */}
          {selItem && (
            <Card s={{ border:`1px solid #FFE60030` }}>
              <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:10 }}>📦 Anúncio Selecionado</div>
              <div style={{ display:"flex", gap:10, alignItems:"center", marginBottom:14 }}>
                {selItem.thumbnail && <img src={selItem.thumbnail} alt="" style={{ width:48, height:48, borderRadius:10, objectFit:"cover" }}/>}
                <div style={{ flex:1 }}>
                  <div style={{ color:C.t1, fontWeight:600, fontSize:12 }}>{selItem.title}</div>
                  <div style={{ color:C.neon, fontSize:12, marginTop:2 }}>R$ {selItem.price}</div>
                  <div style={{ color:selItem.status==="active"?C.neon:C.red, fontSize:10, marginTop:1 }}>
                    {selItem.status==="active"?"🟢 Ativo":"🔴 "+selItem.status}
                    {selItem.hasVideo && <span style={{ marginLeft:8, color:C.blue }}>🎬 Tem vídeo</span>}
                  </div>
                </div>
                <button onClick={()=>{setSelItem(null);setSelVideo(null);setValidation(null);}}
                  style={{ background:`${C.red}15`, border:`1px solid ${C.red}30`, borderRadius:8, padding:"5px 10px", color:C.red, fontWeight:700, fontSize:11, cursor:"pointer" }}>✕</button>
              </div>

              {/* Video selector */}
              <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:8 }}>🎬 Selecionar Vídeo</div>
              {(videos||[]).length === 0 ? (
                <div style={{ background:C.s1, borderRadius:10, padding:"12px 14px", color:C.t3, fontSize:12, textAlign:"center" }}>
                  Nenhum vídeo cadastrado. Adicione vídeos em <strong style={{color:C.t1}}>✨ Criar → 📹 Meu Vídeo</strong>
                </div>
              ) : (
                <div style={{ display:"flex", flexDirection:"column", gap:8, maxHeight:240, overflowY:"auto" }}>
                  {(videos||[]).map(v => {
                    const prod = (links||[]).find(l=>l.id===v.productId);
                    const isSel = selVideo?.id === v.id;
                    return (
                      <div key={v.id} onClick={()=>{setSelVideo(v);validateVideo(v);}}
                        style={{ display:"flex", gap:10, alignItems:"center", padding:"10px 12px", background:isSel?`${C.neon}12`:C.s1, border:`1px solid ${isSel?C.neon+"40":C.b1}`, borderRadius:12, cursor:"pointer" }}>
                        <div style={{ width:40, height:40, borderRadius:9, background:C.b1, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>🎬</div>
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ color:C.t1, fontSize:12, fontWeight:600, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                            {v.url?.split('/').pop()?.slice(0,30) || "Vídeo "+v.id}
                          </div>
                          {prod && <div style={{ color:C.t3, fontSize:10, marginTop:1 }}>📦 {prod.productName}</div>}
                          <div style={{ color:C.t3, fontSize:10 }}>{new Date(v.createdAt||Date.now()).toLocaleDateString("pt-BR")}</div>
                        </div>
                        {isSel && <div style={{ color:C.neon, fontSize:16, flexShrink:0 }}>✅</div>}
                      </div>
                    );
                  })}
                </div>
              ...
              {/* Validation results */}
              {validation && selVideo && (
                <div style={{ marginTop:12, background:C.s1, borderRadius:12, padding:"12px 14px" }}>
                  <div style={{ color:C.t1, fontWeight:700, fontSize:12, marginBottom:8 }}>🔍 Validação do Vídeo</div>
                  {(validation.checks||[]).map((ch, i) => (
                    <div key={i} style={{ display:"flex", gap:8, alignItems:"center", padding:"4px 0" }}>
                      <span style={{ fontSize:14 }}>{ch.ok === true ? "✅" : ch.ok === false ? "❌" : "⚠️"}</span>
                      <span style={{ color:C.t2, fontSize:11 }}>{ch.msg}</span>
                    </div>
                  ))}
                  {validation.requirements && (
                    <div style={{ marginTop:10, background:`${C.blue}10`, border:`1px solid ${C.blue}20`, borderRadius:8, padding:"8px 10px" }}>
                      <div style={{ color:C.blue, fontSize:10, fontWeight:700, marginBottom:4 }}>📋 Requisitos Oficiais ML:</div>
                      <div style={{ color:C.t3, fontSize:10, lineHeight:1.8 }}>
                        Formato: {validation.requirements.format} · 
                        Duração: {validation.requirements.minDuration}–{validation.requirements.maxDuration} · 
                        Resolução: {validation.requirements.minResolution} · 
                        Tamanho máx: {validation.requirements.maxSize}
                      </div>
                    </div>
                  )}
                </div>
              ...
              {jobStatus && (
                <div style={{ marginTop:12, background:C.s1, borderRadius:12, padding:"12px 14px" }}>
                  <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:8 }}>
                    {jobStatus.status === "completed" ? <span style={{fontSize:20}}>✅</span> :
                     jobStatus.status === "failed"    ? <span style={{fontSize:20}}>❌</span> :
                     <Spin size={18} color={C.neon} />}
                    <div style={{ color:C.t1, fontWeight:700, fontSize:12 }}>
                      {jobStatus.status === "uploading"  ? "Enviando vídeo..." :
                       jobStatus.status === "completed"  ? "Concluído!" :
                       jobStatus.status === "unavailable"? "Função indisponível" :
                       jobStatus.status === "failed"     ? "Falhou" : jobStatus.status}
                    </div>
                  </div>
                  {jobStatus.progress > 0 && (
                    <div style={{ background:C.b1, borderRadius:99, height:6, overflow:"hidden" }}>
                      <div style={{ width:`${jobStatus.progress}%`, height:"100%", background:`linear-gradient(90deg,${C.neon},${C.blue})`, borderRadius:99, transition:"width .5s" }}/>
                    </div>
                  )}
                </div>
              )}
              ...
              {selVideo && (
                <button onClick={uploadVideo} disabled={loading}
                  style={{ width:"100%", background:loading?"#1a2540":`linear-gradient(135deg,#FFE600,#FF9900)`, border:"none", borderRadius:13, padding:"14px 0", color:loading?C.t3:"#000", fontWeight:800, fontSize:14, cursor:loading?"default":"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10, marginTop:12 }}>
                  {loading ? <><Spin size={18} color="#000"/> Enviando...</> : "🚀 Enviar Vídeo para ML"}
                </button>
              )}
            </Card>
          )}
          ...
          {!selItem && (
            <Card>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                <div style={{ color:C.t1, fontWeight:700, fontSize:14 }}>📦 Meus Anúncios</div>
                <button onClick={()=>loadItems()} disabled={loading}
                  style={{ background:"#FFE60018", border:"1px solid #FFE60040", borderRadius:9, padding:"5px 12px", color:"#FFE600", fontWeight:700, fontSize:11, cursor:"pointer" }}>
                  {loading ? "..." : "🔄 Atualizar"}
                </button>
              </div>
              ...
              {loading && !items.length && (
                <div style={{ textAlign:"center", padding:24 }}>
                  <Spin size={28} color="#FFE600"/><div style={{ color:C.t3, fontSize:12, marginTop:10 }}>Carregando anúncios...</div>
                </div>
              )}

              {!loading && !items.length && account && (
              ...
              {!account && !loading && (
                <button onClick={loadAccount}
                  style={{ width:"100%", background:`linear-gradient(135deg,#FFE600,#FF9900)`, border:"none", borderRadius:13, padding:"13px 0", color:"#000", fontWeight:800, fontSize:13, cursor:"pointer" }}>
                  🛒 Carregar Anúncios
                </button>
              )}
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                {items.map(item => (
                  <div key={item.id} style={{ display:"flex", gap:10, alignItems:"center", padding:"12px 12px", background:C.s1, border:`1px solid ${C.b1}`, borderRadius:14 }}>
                    {item.thumbnail && <img src={item.thumbnail} alt="" style={{ width:48, height:48, borderRadius:10, objectFit:"cover", flexShrink:0 }}/>}
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ color:C.t1, fontWeight:600, fontSize:12, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{item.title}</div>
                      <div style={{ display:"flex", gap:6, marginTop:3, flexWrap:"wrap" }}>
                        <span style={{ color:C.neon, fontSize:11, fontWeight:700 }}>R$ {item.price}</span>
                        <span style={{ color:item.status==="active"?C.neon:C.t3, fontSize:10 }}>
                          {item.status==="active"?"🟢 Ativo":"🔴 "+item.status}
                        </span>
                        {item.hasVideo && <span style={{ color:C.blue, fontSize:10 }}>🎬 Com vídeo</span>}
                      </div>
                      <div style={{ color:C.t3, fontSize:10, marginTop:1 }}>{item.id}</div>
                    </div>
                    <button onClick={()=>setSelItem(item)}
                      style={{ background:`linear-gradient(135deg,#FFE600,#FF9900)`, border:"none", borderRadius:9, padding:"8px 12px", color:"#000", fontWeight:700, fontSize:11, cursor:"pointer", flexShrink:0 }}>
                      Selecionar
                    </button>
                  </div>
                ))}
      ...
      {subView === "history" && (
        <Card>
          <div style={{ color:C.t1, fontWeight:700, fontSize:14, marginBottom:12 }}>📋 Histórico de Envios</div>
          {history.length === 0 ? (
            <div style={{ textAlign:"center", padding:24, color:C.t3, fontSize:12 }}>Nenhum envio realizado ainda.</div>
          ) : (
            history.map((h, i) => (
              <div key={i} style={{ padding:"12px 0", borderBottom:i<history.length-1?`1px solid ${C.b1}`:"none" }}>
                <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:4 }}>
                  <span style={{ fontSize:16 }}>{h.status==="success"?"🟢":"🔴"}</span>
                  <div style={{ flex:1 }}>
                    <div style={{ color:C.t1, fontWeight:600, fontSize:12 }}>{h.itemTitle||h.itemId}</div>
                    <div style={{ color:C.t3, fontSize:10, marginTop:1 }}>
                      {new Date(h.date).toLocaleString("pt-BR")} · {h.itemId}
                    </div>
                  </div>
                </div>
                {h.error && <div style={{ color:C.red, fontSize:11, background:`${C.red}10`, borderRadius:8, padding:"5px 9px" }}>{h.error}</div>}
                {h.message && !h.error && <div style={{ color:C.neon, fontSize:11 }}>{h.message}</div>}
              </div>
            ))
          )}
        </Card>
      ...
      {/* ── SUCCESS ── */}
      {subView === "success" && (
        <Card glow={C.neon} s={{ textAlign:"center", padding:"32px 20px", background:`${C.neon}08` }}>
          <div style={{ fontSize:56, marginBottom:14 }}>🎉</div>
          <div style={{ color:C.neon, fontWeight:900, fontSize:20, marginBottom:8 }}>Vídeo enviado!</div>
          <div style={{ color:C.t2, fontSize:13, lineHeight:1.7, marginBottom:20 }}>
            O Mercado Livre está processando o vídeo.<br/>Pode levar alguns minutos para aparecer no anúncio.
          </div>
          <button onClick={()=>{setSubView("home");setSelItem(null);setSelVideo(null);setJobStatus(null);}}
            style={{ background:`linear-gradient(135deg,${C.neon},${C.blue})`, border:"none", borderRadius:13, padding:"13px 24px", color:"#000", fontWeight:800, fontSize:13, cursor:"pointer" }}>
            ← Voltar ao início
          </button>
        </Card>
      )}
...
// ══════════════════════════════════════════════════════
// YOUTUBE PUBLISHER — Publicação automática via API oficial
// ══════════════════════════════════════════════════════
function YouTubePublisher({ videos, links, queue, setQueue }) {
  const YT_KEY    = "yt_oauth_tokens";
  const loadTokens = () => { try { return JSON.parse(localStorage.getItem(YT_KEY)||"null"); } catch { return null; } };
  const saveTokens = (t) => { try { localStorage.setItem(YT_KEY, JSON.stringify(t)); } catch {} };

  const [tokens, setTokens]   = useState(loadTokens);
  const [channel, setChannel] = useState(null);
  const [selVideo, setSelVid] = useState(null);
  const [form, setForm]       = useState({ title:"", description:"", privacy:"public", tags:"", scheduledAt:"" });
  const [status, setStatus]   = useState(null); // {state, msg, url, videoId}
  const [loading, setLoading] = useState(false);
  const [history, setHistory] = useState(() => { try { return JSON.parse(localStorage.getItem("yt_pub_history")||"[]"); } catch { return []; } });
  const [view, setView]       = useState("home"); // home | publish | history

  const isExpired = (t) => t && Date.now() > (t.expires - 60000);
  ...
  const ensureToken = async () => {
    ...
    setLoading(true);
    try {
      const r = await fetch("/api/youtube-publish?action=refresh", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ refresh_token: tokens.refresh_token })
      });
      const data = await r.json();
      if (data.ok) {
        const newT = {...tokens, access_token:data.token, expires:data.expires};
        setTokens(newT); saveTokens(newT);
        return data.token;
      }
      setTokens(null); saveTokens(null);
      return null;
  ...
  const connect = async () => {
    setLoading(true);
    try {
      const r = await fetch("/api/youtube-publish?action=auth-url", { method:"POST", headers:{"Content-Type":"application/json"}, body:"{}" });
      const data = await r.json();
      if (data.ok) window.open(data.url, "_blank");
      else setStatus({ state:"error", msg: data.error });
  ...
  const loadChannel = async (token) => {
    try {
      const r = await fetch("/api/youtube-publish?action=channel", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ token })
      });
      const data = await r.json();
      if (data.ok) setChannel(data.channel);
    } catch {}
  };
  ...
  useState(() => {
    ...
    const yt_token   = params.get("yt_token");
    const yt_refresh = params.get("yt_refresh");
    const yt_expires = params.get("yt_expires");
    const yt_error   = params.get("yt_error");
    if (yt_token) {
      const t = { access_token:yt_token, refresh_token:yt_refresh, expires:Number(yt_expires) };
      setTokens(t); saveTokens(t);
      loadChannel(yt_token);
      window.history.replaceState({}, "", window.location.pathname);
      setStatus({ state:"success", msg:"✅ Conta YouTube conectada com sucesso!" });
    }
    if (yt_error) {
      setStatus({ state:"error", msg:"Erro ao conectar: "+yt_error });
      window.history.replaceState({}, "", window.location.pathname);
    }
    if (yt_token && !channel) loadChannel(yt_token);
  });

  useEffect(() => {
    if (tokens && !channel) ensureToken().then(t => { if(t) loadChannel(t); });
  }, []);

  // ── Publish video ──
  const publish = async () => {
    if (!selVideo) return setStatus({ state:"error", msg:"Selecione um vídeo primeiro" });
    setLoading(true);
    setStatus({ state:"uploading", msg:"⏳ Enviando vídeo para o YouTube..." });
    try {
      const token = await ensureToken();
      if (!token) {
        setStatus({ state:"error", msg:"Precisamos conectar sua conta novamente para continuar publicando." });
        setLoading(false); return;
      }
      const link = (links||[]).find(l => l.id === selVideo.productId);
      const desc = form.description || (link ? `${link.productName}

🔗 ${link.trackedUrl||link.originalUrl}

✅ Link de afiliado — ganho comissão se você comprar pelo link.` : "");
      const r = await fetch("/api/youtube-publish?action=upload", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({
          token,
          videoUrl:    selVideo.url,
          title:       form.title || link?.productName || "Oferta incrível!",
          description: desc,
          tags:        form.tags ? form.tags.split(",").map(t=>t.trim()) : ["afiliado","oferta","promoção"],
      ...
      const data = await r.json();
      if (data.ok) {
        const entry = { id:data.videoId, url:data.url, title:form.title||link?.productName, date:new Date().toISOString(), status:"published", scheduled:data.scheduled };
        const newHistory = [entry, ...history].slice(0,50);
        setHistory(newHistory);
        localStorage.setItem("yt_pub_history", JSON.stringify(newHistory));
        setStatus({ state:"success", msg: data.scheduled ? `⏳ Publicação agendada para ${new Date(data.scheduled).toLocaleString("pt-BR")}` : "✅ Vídeo publicado no YouTube com sucesso!", url:data.url, videoId:data.videoId });
        // Add to app queue as "done"
        if (setQueue) {
          setQueue(q => [...(q||[]), { id:Date.now().toString(), platform:"youtube", linkId:selVideo.productId, status:"done", postedAt:new Date().toISOString(), ytVideoId:data.videoId, ytUrl:data.url }]);
        }
      } else {
        setStatus({ state:"error", msg:data.error || "Erro ao publicar" });
        const entry = { title:form.title, date:new Date().toISOString(), status:"error", error:data.error };
        const newH = [entry,...history].slice(0,50);
        setHistory(newH); localStorage.setItem("yt_pub_history",JSON.stringify(newH));
      }
    } catch(e) { setStatus({ state:"error", msg:e.message }); }
    setLoading(false);
  };

  const prod = selVideo ? (links||[]).find(l=>l.id===selVideo.productId) : null;

  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>

      {/* Header */}
      <Card glow="#FF0000" s={{ background:"linear-gradient(135deg,#FF000010,#FF000005)", border:"1px solid #FF000030" }}>
        <div style={{ display:"flex", gap:12, alignItems:"center" }}>
          <div style={{ width:50, height:50, borderRadius:14, background:"#FF0000", display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, flexShrink:0 }}>▶️</div>
          <div style={{ flex:1 }}>
            <div style={{ color:C.t1, fontWeight:900, fontSize:15 }}>YouTube — Publicação Automática</div>
            <div style={{ color:C.t3, fontSize:11, marginTop:2 }}>Publica vídeos diretamente via API oficial do YouTube</div>
          </div>
        </div>
        <div style={{ display:"flex", gap:5, marginTop:12 }}>
          {[{id:"home",lbl:"🏠 Início"},{id:"publish",lbl:"🚀 Publicar"},{id:"history",lbl:"📋 Histórico"}].map(t=>(
            <button key={t.id} onClick={()=>setView(t.id)}
              style={{ flex:1, padding:"7px 0", borderRadius:10, border:`1px solid ${view===t.id?"#FF000050":C.b1}`, background:view===t.id?"#FF000018":"transparent", color:view===t.id?"#FF0000":C.t3, fontWeight:700, fontSize:10, cursor:"pointer" }}>
              {t.lbl}
            </button>
          ))}
        </div>
      ...
      {/* Status message */}
      {status && (
        <div style={{ background:status.state==="error"?`${C.red}15`:status.state==="success"?`${C.neon}15`:`${C.blue}15`, border:`1px solid ${status.state==="error"?C.red:status.state==="success"?C.neon:C.blue}30`, borderRadius:12, padding:"11px 14px" }}>
          <div style={{ color:status.state==="error"?C.red:status.state==="success"?C.neon:C.blue, fontSize:13, fontWeight:700 }}>{status.msg}</div>
          {status.url && <a href={status.url} target="_blank" rel="noreferrer" style={{ color:C.blue, fontSize:11, display:"block", marginTop:4 }}>▶️ Ver no YouTube →</a>}
          {status.state==="error" && status.msg.includes("conectar") && (
            <button onClick={connect} style={{ marginTop:8, background:"#FF0000", border:"none", borderRadius:9, padding:"8px 16px", color:"#fff", fontWeight:700, fontSize:12, cursor:"pointer" }}>
              🔄 Conectar Novamente
            </button>
          )}
          <button onClick={()=>setStatus(null)} style={{ marginTop:6, background:"transparent", border:"none", color:C.t3, fontSize:10, cursor:"pointer", display:"block" }}>Fechar ×</button>
        </div>
      )}
        ...
        <>
          {!tokens ? (
            <Card s={{ textAlign:"center", padding:"28px 16px" }}>
              ...
              <div style={{ color:C.t1, fontWeight:800, fontSize:15, marginBottom:8 }}>Conecte sua conta YouTube</div>
              <div style={{ color:C.t3, fontSize:12, lineHeight:1.8, marginBottom:16 }}>
                Publique vídeos diretamente no seu canal sem precisar abrir o YouTube.
              </div>
              <div style={{ background:`${C.gold}10`, border:`1px solid ${C.gold}25`, borderRadius:11, padding:"10px 14px", marginBottom:16, textAlign:"left" }}>
                <div style={{ color:C.gold, fontSize:11, lineHeight:1.9 }}>
                  <strong style={{color:C.t1}}>Para conectar:</strong><br/>
                  1. Configure YOUTUBE_CLIENT_ID no Vercel<br/>
                  2. Configure YOUTUBE_CLIENT_SECRET no Vercel<br/>
                  3. Toque em Conectar abaixo<br/>
                  4. Faça login com sua conta Google<br/>
                  5. Autorize o acesso ao YouTube
                </div>
              </div>
              <button onClick={connect} disabled={loading}
                style={{ width:"100%", background:"#FF0000", border:"none", borderRadius:13, padding:"14px 0", color:"#fff", fontWeight:800, fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
                {loading ? <><Spin size={18} color="#fff"/> Carregando...</> : "▶️ Conectar com YouTube"}
              </button>
            </Card>
          ) : (
            <Card s={{ border:"1px solid #FF000030" }}>
              <div style={{ display:"flex", gap:12, alignItems:"center" }}>
                {channel?.thumb && <img src={channel.thumb} alt="" style={{ width:44, height:44, borderRadius:"50%", objectFit:"cover" }}/>}
                <div style={{ flex:1 }}>
                  <div style={{ color:C.t1, fontWeight:700, fontSize:14 }}>{channel?.name || "Canal conectado"}</div>
                  <div style={{ color:C.neon, fontSize:11, marginTop:2 }}>✅ YouTube conectado</div>
                  {channel?.subs && <div style={{ color:C.t3, fontSize:10, marginTop:1 }}>{Number(channel.subs).toLocaleString("pt-BR")} inscritos</div>}
                </div>
                <button onClick={()=>{setTokens(null);saveTokens(null);setChannel(null);}}
                  style={{ background:`${C.red}15`, border:`1px solid ${C.red}30`, borderRadius:9, padding:"6px 12px", color:C.red, fontWeight:700, fontSize:11, cursor:"pointer" }}>
                  Desconectar
                </button>
              </div>
              <button onClick={()=>setView("publish")}
                style={{ width:"100%", background:"#FF0000", border:"none", borderRadius:13, padding:"13px 0", color:"#fff", fontWeight:800, fontSize:14, cursor:"pointer", marginTop:14 }}>
          ...
          <Card s={{ background:`${C.blue}08`, border:`1px solid ${C.blue}20` }}>
            <div style={{ color:C.blue, fontWeight:700, fontSize:13, marginBottom:10 }}>ℹ️ Como funciona</div>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {["1️⃣ Conecte sua conta Google/YouTube","2️⃣ Selecione um vídeo do app","3️⃣ Escolha título, descrição e privacidade","4️⃣ Publique agora ou agende para depois","5️⃣ O app publica direto — sem abrir o YouTube!"].map((s,i)=>(
                <div key={i} style={{ color:C.t2, fontSize:12, padding:"5px 0", borderBottom:i<4?`1px solid ${C.b1}`:"none" }}>{s}</div>
              ))}
            </div>
            <div style={{ marginTop:12, background:`${C.orange}10`, border:`1px solid ${C.orange}25`, borderRadius:10, padding:"9px 12px", color:C.orange, fontSize:11 }}>
              ⚠️ Requer configuração de YOUTUBE_CLIENT_ID e YOUTUBE_CLIENT_SECRET no Vercel (Google Cloud Console).
            </div>
          </Card>
        </>
      )}
        ...
        <>
          {!tokens ? (
            <Card s={{ textAlign:"center", padding:24 }}>
              <div style={{ color:C.t1, fontWeight:700, marginBottom:12 }}>Conecte sua conta primeiro</div>
              <button onClick={()=>setView("home")} style={{ background:`${C.blue}15`, border:`1px solid ${C.blue}30`, borderRadius:11, padding:"10px 20px", color:C.blue, fontWeight:700, fontSize:13, cursor:"pointer" }}>
                ← Ir para Início
              </button>
            </Card>
          ) : (
            <>
              {/* Video selector */}
              <Card>
                <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:10 }}>🎬 Selecionar Vídeo</div>
                {(videos||[]).length === 0 ? (
                  <div style={{ background:C.s1, borderRadius:10, padding:"12px 14px", color:C.t3, fontSize:12, textAlign:"center" }}>
                    Nenhum vídeo cadastrado. Adicione em ✨ Criar → 📹 Meu Vídeo
                  </div>
                ) : (
                  <div style={{ display:"flex", flexDirection:"column", gap:8, maxHeight:220, overflowY:"auto" }}>
                    {(videos||[]).map(v=>{
                      const lk = (links||[]).find(l=>l.id===v.productId);
                      const sel = selVideo?.id===v.id;
                      return (
                        <div key={v.id} onClick={()=>{ setSelVid(v); setForm(f=>({...f, title:lk?.productName||"", description:""})); }}
                          style={{ display:"flex", gap:10, alignItems:"center", padding:"10px 12px", background:sel?`${C.neon}12`:C.s1, border:`1px solid ${sel?C.neon+"40":C.b1}`, borderRadius:12, cursor:"pointer" }}>
                          <div style={{ width:38, height:38, borderRadius:9, background:"#FF000020", display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>▶️</div>
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ color:C.t1, fontSize:12, fontWeight:600, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{v.url?.split("/").pop()?.slice(0,30)||"Vídeo"}</div>
                            {lk && <div style={{ color:C.t3, fontSize:10 }}>📦 {lk.productName}</div>}
                          </div>
                          {sel && <span style={{ color:C.neon, fontSize:16 }}>✅</span>}
                        </div>
                      );
                    })}
                  </div>
              ...
              {selVideo && (
                <Card>
                  <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:12 }}>📝 Detalhes do Vídeo</div>

                  <div style={{ color:C.t3, fontSize:10, fontWeight:700, textTransform:"uppercase", marginBottom:4 }}>Título</div>
                  <input value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))}
                    placeholder="Título do vídeo..."
                    style={{ width:"100%", background:C.s1, border:`1px solid ${C.b1}`, borderRadius:10, padding:"9px 12px", color:C.t1, fontSize:12, outline:"none", marginBottom:10, boxSizing:"border-box" }}/>

                  <div style={{ color:C.t3, fontSize:10, fontWeight:700, textTransform:"uppercase", marginBottom:4 }}>Descrição</div>
                  <textarea value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))}
                    placeholder="Descrição do vídeo... (link de afiliado será adicionado automaticamente)"
                    rows={3}
                    style={{ width:"100%", background:C.s1, border:`1px solid ${C.b1}`, borderRadius:10, padding:"9px 12px", color:C.t1, fontSize:12, outline:"none", resize:"none", fontFamily:"inherit", marginBottom:10, boxSizing:"border-box" }}/>

                  <div style={{ color:C.t3, fontSize:10, fontWeight:700, textTransform:"uppercase", marginBottom:4 }}>Tags (separadas por vírgula)</div>
                  <input value={form.tags} onChange={e=>setForm(f=>({...f,tags:e.target.value}))}
                    placeholder="afiliado, oferta, promoção..."
                    style={{ width:"100%", background:C.s1, border:`1px solid ${C.b1}`, borderRadius:10, padding:"9px 12px", color:C.t1, fontSize:12, outline:"none", marginBottom:10, boxSizing:"border-box" }}/>

                  <div style={{ color:C.t3, fontSize:10, fontWeight:700, textTransform:"uppercase", marginBottom:6 }}>Privacidade</div>
                  <div style={{ display:"flex", gap:6, marginBottom:12 }}>
                    {[["public","🌍 Público"],["unlisted","🔗 Não listado"],["private","🔒 Privado"]].map(([v,l])=>(
                      <button key={v} onClick={()=>setForm(f=>({...f,privacy:v}))}
                        style={{ flex:1, padding:"7px 0", borderRadius:9, border:`1px solid ${form.privacy===v?"#FF000050":C.b1}`, background:form.privacy===v?"#FF000018":"transparent", color:form.privacy===v?"#FF0000":C.t3, fontWeight:700, fontSize:10, cursor:"pointer" }}>
                        {l}
                      </button>
                    ))}
                  </div>

                  <div style={{ color:C.t3, fontSize:10, fontWeight:700, textTransform:"uppercase", marginBottom:4 }}>⏰ Agendar para (opcional)</div>
                  <input type="datetime-local" value={form.scheduledAt} onChange={e=>setForm(f=>({...f,scheduledAt:e.target.value}))}
                    style={{ width:"100%", background:C.s1, border:`1px solid ${C.b1}`, borderRadius:10, padding:"9px 12px", color:C.t1, fontSize:12, outline:"none", marginBottom:12, boxSizing:"border-box" }}/>

                  {form.scheduledAt && (
                    <div style={{ background:`${C.gold}10`, border:`1px solid ${C.gold}25`, borderRadius:10, padding:"8px 12px", color:C.gold, fontSize:11, marginBottom:12 }}>
                      ⏳ Agendado para {new Date(form.scheduledAt).toLocaleString("pt-BR")}
                    </div>
                  )}

                  <button onClick={publish} disabled={loading}
                    style={{ width:"100%", background:loading?"#1a2540":"#FF0000", border:"none", borderRadius:13, padding:"14px 0", color:loading?C.t3:"#fff", fontWeight:800, fontSize:14, cursor:loading?"default":"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
                    {loading ? <><Spin size={18} color="#fff"/>
                      {status?.state==="uploading" ? "Enviando vídeo..." : "Processando..."}
                    </> : form.scheduledAt ? "⏰ Agendar Publicação" : "🚀 Publicar Agora no YouTube"}
                  </button>
                </Card>
              )}
            </>
      ...
      {view==="history" && (
        <Card>
          <div style={{ color:C.t1, fontWeight:700, fontSize:14, marginBottom:12 }}>📋 Histórico de Publicações</div>
          {history.length === 0 ? (
            <div style={{ textAlign:"center", padding:24, color:C.t3, fontSize:12 }}>Nenhuma publicação ainda.</div>
          ) : (
            history.map((h,i)=>(
              <div key={i} style={{ padding:"11px 0", borderBottom:i<history.length-1?`1px solid ${C.b1}`:"none" }}>
                <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:3 }}>
                  <span style={{ fontSize:16 }}>{h.status==="published"||h.status==="success"?"✅":h.status==="error"?"❌":"⏳"}</span>
                  <div style={{ flex:1 }}>
                    <div style={{ color:C.t1, fontWeight:600, fontSize:12 }}>{h.title||"Vídeo"}</div>
                    <div style={{ color:C.t3, fontSize:10 }}>{new Date(h.date).toLocaleString("pt-BR")}</div>
                  </div>
                </div>
                {h.url && <a href={h.url} target="_blank" rel="noreferrer" style={{ color:"#FF0000", fontSize:11 }}>▶️ Ver no YouTube →</a>}
                {h.error && <div style={{ color:C.red, fontSize:11, marginTop:3 }}>{h.error}</div>}
                {h.scheduled && <div style={{ color:C.gold, fontSize:10, marginTop:2 }}>⏳ Agendado: {new Date(h.scheduled).toLocaleString("pt-BR")}</div>}
              </div>
            ))
          )}
        </Card>
...
// ══════════════════════════════════════════════════════
// ADMIN PANEL — Acesso: login=root senha=toor
// Tutoriais + Testes de API + Ferramentas
// ══════════════════════════════════════════════════════
function AdminPanel({ onClose }) {
  const [adminTab, setAdminTab]     = useState("guias");
  ...
  const log = (msg, type="info") => {
    ...
    setLogLines(l => [`[${ts}] ${type==="ok"?"✅":type==="err"?"❌":"ℹ️"} ${msg}`, ...l].slice(0,50));
  };

  // ── API TESTS ──
  const tests = [
    {
      id:"groq", name:"GROQ — IA ARIA", color:"#9b72f7", ico:"🤖",
      desc:"Testa se a chave GROQ está funcionando e a ARIA responde",
      run: async () => {
        const r = await fetch("/api/ai-manager", {
          method:"POST", headers:{"Content-Type":"application/json"},
          body: JSON.stringify({ prompt:"Diga apenas: OK FUNCIONANDO", type:"test" })
        });
        const d = await r.json();
        return d.result ? { ok:true, msg:`Resposta: ${d.result.slice(0,60)}` } : { ok:false, msg: d.error||"Sem resposta" };
      }
    },
    {
      id:"telegram", name:"Telegram Bot", color:"#2AABEE", ico:"✈️",
      desc:"Envia mensagem de teste no seu canal Telegram",
      run: async () => {
        const r = await fetch("/api/telegram-bot", {
          method:"POST", headers:{"Content-Type":"application/json"},
          body: JSON.stringify({ text:"🧪 *Teste AfiliadoAI Admin* — Bot funcionando! ✅" })
        });
        const d = await r.json();
        return d.ok ? { ok:true, msg:`Mensagem enviada! ID: ${d.message_id}` } : { ok:false, msg: d.error||"Falha" };
      }
    },
    {
      id:"supabase", name:"Supabase DB", color:"#3ECF8E", ico:"☁️",
      desc:"Verifica conexão com o banco de dados Supabase",
      run: async () => {
        ...
        const d = await r.json();
        return d.ok ? { ok:true, msg:"Banco conectado!" } : { ok:false, msg: d.error||"Sem conexão" };
      }
    },
    {
      id:"ml", name:"Mercado Livre API", color:"#FFE600", ico:"🛒",
      desc:"Busca produto de teste na API pública do ML",
      run: async () => {
        const r = await fetch("https://api.mercadolibre.com/sites/MLB/search?q=smartphone&limit=1");
        const d = await r.json();
        const item = d.results?.[0];
        return item ? { ok:true, msg:`API OK — Exemplo: ${item.title?.slice(0,40)}` } : { ok:false, msg:"Sem resultados" };
      }
    },
    {
      id:"youtube", name:"YouTube API", color:"#FF0000", ico:"▶️",
      desc:"Verifica se as credenciais YouTube estão configuradas",
      run: async () => {
        const r = await fetch("/api/youtube-publish?action=auth-url", {
          method:"POST", headers:{"Content-Type":"application/json"}, body:"{}"
        });
        const d = await r.json();
        return d.ok ? { ok:true, msg:"YouTube configurado! URL OAuth gerada." } : { ok:false, msg: d.error||"YOUTUBE_CLIENT_ID não configurado" };
      }
    },
    {
      id:"did", name:"D-ID Avatar IA", color:"#9b72f7", ico:"🧑‍💻",
      desc:"Verifica se a chave D-ID está configurada",
      run: async () => {
        const r = await fetch("/api/create-video?action=avatar", {
          method:"POST", headers:{"Content-Type":"application/json"},
          body: JSON.stringify({ script:"Teste", avatar:"ana", lang:"pt-BR" })
        });
        const d = await r.json();
        return d.jobId ? { ok:true, msg:`D-ID OK! Job: ${d.jobId}` } : { ok:false, msg: d.error||"DID_API_KEY não configurada" };
      }
    },
    {
      id:"spy", name:"Espião ML", color:"#f05c5c", ico:"🕵️",
      desc:"Testa busca de ofertas no Mercado Livre",
      run: async () => {
        const r = await fetch("https://api.mercadolibre.com/sites/MLB/search?category=MLB1051&sort=relevance&limit=3");
        const d = await r.json();
        const count = d.results?.filter(i=>i.original_price&&i.original_price>i.price).length||0;
        return { ok:true, msg:`Espião OK — ${count} produtos com desconto encontrados` };
      }
    },
  ];

  const runTest = async (test) => {
    setTesting(t=>({...t,[test.id]:true}));
    setTestResult(r=>({...r,[test.id]:null}));
    log(`Testando ${test.name}...`);
    try {
      const result = await test.run();
      setTestResult(r=>({...r,[test.id]:result}));
      log(`${test.name}: ${result.msg}`, result.ok?"ok":"err");
    } catch(e) {
      setTestResult(r=>({...r,[test.id]:{ok:false,msg:e.message}}));
      log(`${test.name}: ${e.message}`, "err");
    }
    setTesting(t=>({...t,[test.id]:false}));
  };

  const runAll = async () => {
    log("Iniciando teste de todas as APIs...");
  ...
  const GUIDES = [
    {
      id:"env", ico:"⚙️", title:"Variáveis de Ambiente", color:C.neon,
      sections:[
        { title:"🔴 Obrigatórias", items:[
          { name:"GROQ_API_KEY", where:"console.groq.com → API Keys", format:"gsk_xxx..." },
          { name:"TELEGRAM_BOT_TOKEN", where:"@BotFather → /newbot", format:"123456:AABBcc..." },
          { name:"TELEGRAM_CHAT_ID", where:"@userinfobot → encaminhar msg do canal", format:"-1001234567890" },
          { name:"SUPABASE_URL", where:"supabase.com → Settings → API", format:"https://xxx.supabase.co" },
          { name:"SUPABASE_ANON_KEY", where:"supabase.com → Settings → API", format:"eyJhbGci..." },
          { name:"DISABLE_ESLINT_PLUGIN", where:"Valor fixo", format:"true" },
        ]},
        { title:"🟡 Mercado Livre", items:[
          { name:"ML_CLIENT_ID", where:"developers.mercadolivre.com.br", format:"1234567890" },
          { name:"ML_CLIENT_SECRET", where:"developers.mercadolivre.com.br", format:"SuaSecret..." },
          { name:"ML_REDIRECT_URI", where:"URL do app + /", format:"https://turbo-pancake-omega.vercel.app/" },
          { name:"REACT_APP_ML_CLIENT_ID", where:"Mesmo valor do ML_CLIENT_ID", format:"1234567890" },
        ]},
        { title:"▶️ YouTube", items:[
          { name:"YOUTUBE_CLIENT_ID", where:"console.cloud.google.com", format:"xxx.apps.googleusercontent.com" },
          { name:"YOUTUBE_CLIENT_SECRET", where:"console.cloud.google.com", format:"GOCSPX-xxx" },
          { name:"YOUTUBE_REDIRECT_URI", where:"URL app + /api/youtube-publish?action=callback", format:"https://turbo-pancake-omega.vercel.app/api/youtube-publish?action=callback" },
          { name:"APP_URL", where:"URL base do app sem barra", format:"https://turbo-pancake-omega.vercel.app" },
        ]},
        { title:"🟣 Opcionais", items:[
          { name:"DID_API_KEY", where:"d-id.com → Settings → API", format:"Basic xxx..." },
          { name:"JSON2VIDEO_API_KEY", where:"json2video.com → Settings → API", format:"xxx..." },
        ]},
      ]
    },
    {
      id:"meta", ico:"📘", title:"Meta (Facebook + Instagram)", color:"#1877F2",
      sections:[
        { title:"Pré-requisitos", items:[
          { name:"Página do Facebook", where:"facebook.com/pages/create", format:"Obrigatório — não funciona com perfil pessoal" },
          { name:"Instagram Profissional", where:"IG → Configurações → Conta → Profissional", format:"Business ou Creator" },
          { name:"Conectar IG à Página FB", where:"FB Página → Configurações → Instagram", format:"Obrigatório" },
        ]},
        { title:"Processo de aprovação (2-4 semanas)", items:[
          { name:"1. Criar app Meta", where:"developers.facebook.com → Criar App", format:"Tipo: Outro / Categoria: Empresa" },
          { name:"2. Adicionar produtos", where:"Painel app → Adicionar produto", format:"Instagram Graph API + Facebook Login" },
          { name:"3. Configurar OAuth", where:"Facebook Login → Configurar", format:"Redirect: turbo-pancake-omega.vercel.app/api/meta-publish?action=callback" },
          { name:"4. Solicitar permissões", where:"Análise do App → Solicitar", format:"pages_manage_posts, instagram_content_publish, instagram_basic" },
          { name:"5. Preencher revisão", where:"Descrição + vídeo do app funcionando", format:"Aguardar 2-4 semanas" },
        ]},
        { title:"Variáveis após aprovação", items:[
          { name:"META_APP_ID", where:"Configurações → Básico → ID do App", format:"123456789" },
          { name:"META_APP_SECRET", where:"Configurações → Básico → Chave Secreta", format:"abc123..." },
          { name:"META_REDIRECT_URI", where:"URL callback configurada", format:"https://turbo-pancake-omega.vercel.app/api/meta-publish?action=callback" },
        ]},
      ]
    },
    {
      id:"x", ico:"🐦", title:"X (Twitter)", color:"#eef2f8",
      sections:[
        { title:"Processo (1-3 dias)", items:[
          { name:"1. Criar conta dev", where:"developer.twitter.com → Sign up for Free", format:"Login com conta X existente" },
          { name:"2. Preencher formulário", where:"Descrever uso da API em inglês", format:"Explicar que é para postar conteúdo de afiliados" },
          { name:"3. Criar projeto e app", where:"Dashboard → + Add App", format:"Nome: AfiliadoAI / Ambiente: Production" },
          { name:"4. Configurar permissões", where:"App → Settings → App permissions", format:"Read and Write" },
          { name:"5. Callback URL", where:"App → Settings → Authentication settings", format:"https://turbo-pancake-omega.vercel.app/api/x-publish?action=callback" },
        ]},
        { title:"Variáveis após aprovação", items:[
          { name:"X_API_KEY", where:"Keys and tokens → API Key", format:"xxx..." },
          { name:"X_API_SECRET", where:"Keys and tokens → API Key Secret", format:"xxx..." },
          { name:"X_ACCESS_TOKEN", where:"Keys and tokens → Generate", format:"xxx..." },
          { name:"X_ACCESS_SECRET", where:"Keys and tokens → Access Token Secret", format:"xxx..." },
        ]},
      ]
    },
    {
      id:"supabase_sql", ico:"☁️", title:"Supabase — Criar Tabela", color:"#3ECF8E",
      sections:[
        { title:"SQL para executar no Supabase", items:[
          { name:"Acessar SQL Editor", where:"supabase.com → seu projeto → SQL Editor → New query", format:"" },
          { name:"Colar e executar", where:"Cole o SQL abaixo e clique em Run", format:
            "CREATE TABLE IF NOT EXISTS clicks (\n  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,\n  link_id text,\n  platform text,\n  device text,\n  ip_hash text,\n  clicked_at timestamptz DEFAULT now()\n);\n\nALTER TABLE clicks ENABLE ROW LEVEL SECURITY;\nCREATE POLICY \"allow_insert\" ON clicks FOR INSERT WITH CHECK (true);" },
        ]},
      ]
    },
    {
      id:"github_fix", ico:"⚠️", title:"Corrigir GitHub — 13 APIs", color:C.red,
      sections:[
        { title:"Deletar 3 arquivos antigos (github.com/Zynex777/turbo-pancake)", items:[
          { name:"1. Deletar check-video.js", where:"api/ → check-video.js → 🗑️ → Commit changes", format:"" },
          { name:"2. Deletar ml-refresh.js", where:"api/ → ml-refresh.js → 🗑️ → Commit changes", format:"" },
          { name:"3. Deletar tg-monitor.js", where:"api/ → tg-monitor.js → 🗑️ → Commit changes", format:"" },
          { name:"4. Redeploy automático", where:"Vercel faz deploy automático após cada commit", format:"Aguardar 2-3 minutos" },
        ]},
      ]
    },
  ];
  ...
  return (
    <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:C.bg, zIndex:9999, display:"flex", flexDirection:"column", overflow:"hidden" }}>

      {/* Admin Header */}
      <div style={{ background:`linear-gradient(135deg,${C.red}20,${C.purple}15)`, borderBottom:`1px solid ${C.red}30`, padding:"14px 16px", display:"flex", alignItems:"center", gap:12, flexShrink:0 }}>
        <div style={{ flex:1 }}>
          <div style={{ color:C.red, fontWeight:900, fontSize:16, display:"flex", alignItems:"center", gap:8 }}>
            🔐 Admin Panel
            <span style={{ background:`${C.red}25`, border:`1px solid ${C.red}40`, borderRadius:6, padding:"2px 8px", fontSize:10, color:C.red }}>root</span>
          </div>
          <div style={{ color:C.t3, fontSize:11, marginTop:2 }}>AfiliadoAI PRO — Acesso restrito</div>
        </div>
        <button onClick={onClose}
          style={{ background:`${C.red}20`, border:`1px solid ${C.red}40`, borderRadius:10, padding:"8px 16px", color:C.red, fontWeight:700, fontSize:13, cursor:"pointer" }}>
          ✕ Fechar
        </button>
      </div>

      {/* Admin Tab Bar */}
      <div style={{ display:"flex", gap:0, borderBottom:`1px solid ${C.b1}`, flexShrink:0, overflowX:"auto" }}>
        {[
          {id:"guias",   ico:"📋", lbl:"Guias"},
          {id:"apis",    ico:"🧪", lbl:"Testar APIs"},
          {id:"logs",    ico:"📊", lbl:"Logs"},
          {id:"sistema", ico:"⚙️",  lbl:"Sistema"},
        ].map(t=>(
          <button key={t.id} onClick={()=>setAdminTab(t.id)}
            style={{ flex:1, padding:"12px 4px", border:"none", borderBottom:`2px solid ${adminTab===t.id?C.red:"transparent"}`, background:"transparent", color:adminTab===t.id?C.red:C.t3, fontWeight:700, fontSize:11, cursor:"pointer", whiteSpace:"nowrap" }}>
            {t.ico} {t.lbl}
          </button>
        ))}
      </div>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"16px" }}>
        ...
        {adminTab==="guias" && (
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div style={{ color:C.t1, fontWeight:700, fontSize:15 }}>📋 Guias de Configuração</div>
            {GUIDES.map(guide=>(
              <div key={guide.id} style={{ background:C.card, border:`1px solid ${guide.color}30`, borderRadius:14, overflow:"hidden" }}>
                <div style={{ background:`${guide.color}12`, padding:"12px 14px", display:"flex", gap:10, alignItems:"center" }}>
                  <span style={{ fontSize:22 }}>{guide.ico}</span>
                  <div style={{ color:guide.color, fontWeight:800, fontSize:14 }}>{guide.title}</div>
                </div>
                <div style={{ padding:"12px 14px" }}>
                  {guide.sections.map((sec,si)=>(
                    <div key={si} style={{ marginBottom:12 }}>
                      <div style={{ color:C.t2, fontWeight:700, fontSize:11, textTransform:"uppercase", marginBottom:8, letterSpacing:.8 }}>{sec.title}</div>
                      {sec.items.map((item,ii)=>(
                        <div key={ii} style={{ padding:"8px 0", borderBottom:`1px solid ${C.b1}` }}>
                          <div style={{ color:C.t1, fontSize:12, fontWeight:600 }}>{item.name}</div>
                          {item.where && <div style={{ color:C.t3, fontSize:11, marginTop:2 }}>📍 {item.where}</div>}
                          {item.format && <div style={{ color:C.neon, fontSize:10, fontFamily:"monospace", marginTop:3, background:C.s1, padding:"3px 8px", borderRadius:6, wordBreak:"break-all" }}>{item.format}</div>}
                        </div>
                      ))}
                    </div>
                  ))}
        ...
        {adminTab==="apis" && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.t1, fontWeight:700, fontSize:15 }}>🧪 Testar APIs</div>
              <button onClick={runAll}
                style={{ background:`linear-gradient(135deg,${C.neon},${C.blue})`, border:"none", borderRadius:10, padding:"8px 16px", color:"#000", fontWeight:800, fontSize:12, cursor:"pointer" }}>
                ▶️ Testar Todas
              </button>
            </div>
            ...
            {tests.map(test=>{
              ...
              return (
                <div key={test.id} style={{ background:C.card, border:`1px solid ${result ? (result.ok?C.neon+"30":C.red+"30") : C.b1}`, borderRadius:13, padding:"14px" }}>
                  <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                    <div style={{ width:42, height:42, borderRadius:12, background:`${test.color}20`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0 }}>{test.ico}</div>
                    <div style={{ flex:1 }}>
                      <div style={{ color:C.t1, fontWeight:700, fontSize:13 }}>{test.name}</div>
                      <div style={{ color:C.t3, fontSize:11, marginTop:1 }}>{test.desc}</div>
                      {result && (
                        <div style={{ color:result.ok?C.neon:C.red, fontSize:11, marginTop:4, fontWeight:600 }}>
                          {result.ok?"✅":"❌"} {result.msg}
                        </div>
                      )}
                    </div>
                    <button onClick={()=>runTest(test)} disabled={isLoading}
                      style={{ background:isLoading?"#1a2540":`${test.color}20`, border:`1px solid ${test.color}40`, borderRadius:10, padding:"8px 14px", color:isLoading?C.t3:test.color, fontWeight:700, fontSize:11, cursor:isLoading?"default":"pointer", flexShrink:0 }}>
                      {isLoading ? <Spin size={14} color={test.color}/> : "Testar"}
                    </button>
                  </div>
                </div>
              );
            ...
            {/* Custom API test */}
            <div style={{ background:C.card, border:`1px solid ${C.purple}30`, borderRadius:13, padding:"14px" }}>
              <div style={{ color:C.purple, fontWeight:700, fontSize:13, marginBottom:10 }}>🔧 Teste Personalizado</div>
              <input value={apiInputs.customUrl||""} onChange={e=>setApiInputs(i=>({...i,customUrl:e.target.value}))}
                placeholder="URL da API (ex: /api/ai-manager)"
                style={{ width:"100%", background:C.s1, border:`1px solid ${C.b1}`, borderRadius:9, padding:"8px 12px", color:C.t1, fontSize:12, outline:"none", marginBottom:8, boxSizing:"border-box" }}/>
              <textarea value={apiInputs.customBody||""} onChange={e=>setApiInputs(i=>({...i,customBody:e.target.value}))}
                placeholder={'{"prompt":"teste","type":"test"}'}
                rows={3}
                style={{ width:"100%", background:C.s1, border:`1px solid ${C.b1}`, borderRadius:9, padding:"8px 12px", color:C.t1, fontSize:12, outline:"none", resize:"none", fontFamily:"monospace", marginBottom:8, boxSizing:"border-box" }}/>
              <button onClick={async()=>{
                if (!apiInputs.customUrl) return;
                log(`Testando ${apiInputs.customUrl}...`);
                try {
                  const r = await fetch(apiInputs.customUrl, {
                    method:"POST", headers:{"Content-Type":"application/json"},
                    body: apiInputs.customBody||"{}"
                  });
                  const d = await r.json();
                  setApiInputs(i=>({...i,customResult:JSON.stringify(d,null,2)}));
                  log(`Resposta: ${JSON.stringify(d).slice(0,80)}`, "ok");
                } catch(e) {
                  setApiInputs(i=>({...i,customResult:"Erro: "+e.message}));
                  log(e.message, "err");
                }
              }}
                style={{ background:`${C.purple}20`, border:`1px solid ${C.purple}40`, borderRadius:9, padding:"9px 0", color:C.purple, fontWeight:700, fontSize:12, cursor:"pointer", width:"100%" }}>
                🚀 Executar
              </button>
              {apiInputs.customResult && (
                <div style={{ marginTop:10, background:C.s1, borderRadius:9, padding:"10px 12px", fontFamily:"monospace", fontSize:10, color:C.neon, whiteSpace:"pre-wrap", wordBreak:"break-all", maxHeight:200, overflowY:"auto" }}>
                  {apiInputs.customResult}
                </div>
              )}
            </div>
        ...
        {adminTab==="logs" && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.t1, fontWeight:700, fontSize:15 }}>📊 Console de Logs</div>
              <button onClick={()=>setLogLines([])}
                style={{ background:`${C.red}15`, border:`1px solid ${C.red}30`, borderRadius:9, padding:"6px 14px", color:C.red, fontWeight:700, fontSize:11, cursor:"pointer" }}>
                🗑 Limpar
              </button>
            </div>
            <div style={{ background:"#090f1e", border:`1px solid ${C.b1}`, borderRadius:12, padding:"12px", minHeight:300, fontFamily:"monospace", fontSize:11 }}>
              {logLines.length === 0 ? (
                <div style={{ color:C.t3, textAlign:"center", paddingTop:40 }}>Nenhum log ainda. Execute um teste de API.</div>
              ) : (
                logLines.map((l,i)=>(
                  <div key={i} style={{ color: l.includes("✅")?C.neon:l.includes("❌")?C.red:C.t3, padding:"3px 0", borderBottom:`1px solid ${C.b1}` }}>
                    {l}
                  </div>
                ))
              )}
            ...
            {/* LocalStorage viewer */}
            <div style={{ background:C.card, border:`1px solid ${C.b1}`, borderRadius:13, padding:"14px" }}>
              <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:10 }}>💾 LocalStorage do App</div>
              {["afiliadoai_final_v1","yt_oauth_tokens","ml_video_history","yt_pub_history"].map(key=>{
                const val = (() => { try { const v=localStorage.getItem(key); return v?JSON.parse(v):null; } catch{return null;} })();
                return (
                  <div key={key} style={{ padding:"8px 0", borderBottom:`1px solid ${C.b1}` }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      <div style={{ color:C.neon, fontFamily:"monospace", fontSize:11 }}>{key}</div>
                      <div style={{ color:val?C.t2:C.t3, fontSize:10 }}>{val?"✅ presente":"⚪ vazio"}</div>
                    </div>
                    {val && (
                      <div style={{ color:C.t3, fontSize:10, marginTop:3 }}>
                        {typeof val === "object" ? `${Object.keys(val).length} chaves` : String(val).slice(0,50)}
                      </div>
                    )}
                  </div>
                );
        ...
        {adminTab==="sistema" && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ color:C.t1, fontWeight:700, fontSize:15 }}>⚙️ Sistema</div>

            {/* App info */}
            <div style={{ background:C.card, border:`1px solid ${C.b1}`, borderRadius:13, padding:"14px" }}>
              <div style={{ color:C.t1, fontWeight:700, fontSize:13, marginBottom:10 }}>📱 Informações do App</div>
              {[
                ["Versão",       "AfiliadoAI PRO 2.0"],
                ["Build",        "React 18 + react-scripts 5"],
                ["Host",         "Vercel Hobby (grátis)"],
                ["Banco",        "Supabase + localStorage"],
                ["Storage Key",  "afiliadoai_final_v1"],
                ["APIs",         "10 funções serverless"],
                ["Abas",         "9 abas principais"],
                ["IA",           "Groq Llama 3.3 (grátis)"],
                ["URL",          "turbo-pancake-omega.vercel.app"],
              ].map(([k,v])=>(
                <div key={k} style={{ display:"flex", justifyContent:"space-between", padding:"7px 0", borderBottom:`1px solid ${C.b1}`, fontSize:12 }}>
            ...
            {/* Danger zone */}
            <div style={{ background:`${C.red}08`, border:`1px solid ${C.red}25`, borderRadius:13, padding:"14px" }}>
              <div style={{ color:C.red, fontWeight:700, fontSize:13, marginBottom:10 }}>⚠️ Zona de Perigo</div>
              <button onClick={()=>{
                if (true) { // confirmação via UI
                  localStorage.removeItem("afiliadoai_final_v1");
                  localStorage.removeItem("yt_oauth_tokens");
                  localStorage.removeItem("ml_video_history");
                  localStorage.removeItem("yt_pub_history");
                  alert("✅ Dados apagados! Recarregue a página.");
                }
              }}
                style={{ width:"100%", background:`${C.red}15`, border:`1px solid ${C.red}30`, borderRadius:10, padding:"11px 0", color:C.red, fontWeight:700, fontSize:13, cursor:"pointer", marginBottom:8 }}>
                🗑️ Limpar Todos os Dados Locais
              </button>
              <button onClick={()=>{
                const data = localStorage.getItem("afiliadoai_final_v1");
                if (data) {
                  const blob = new Blob([data], {type:"application/json"});
                  const url = URL.createObjectURL(blob);
                  const a = document.createElement("a");
                  a.href = url; a.download = `afiliadoai_backup_${Date.now()}.json`; a.click();
                  log("Backup exportado!", "ok");
                } else { alert("Nenhum dado para exportar!"); }
              }}
                style={{ width:"100%", background:`${C.blue}15`, border:`1px solid ${C.blue}30`, borderRadius:10, padding:"11px 0", color:C.blue, fontWeight:700, fontSize:13, cursor:"pointer" }}>
                📤 Exportar Backup JSON
              </button>
            </div>

            {/* GitHub fix reminder */}
            <div style={{ background:`${C.gold}08`, border:`1px solid ${C.gold}25`, borderRadius:13, padding:"14px" }}>
              <div style={{ color:C.gold, fontWeight:700, fontSize:13, marginBottom:8 }}>📌 Lembretes Pendentes</div>
              <div style={{ color:C.t2, fontSize:12, lineHeight:2 }}>
                ⚠️ Deletar no GitHub:<br/>
                • api/check-video.js<br/>
                • api/ml-refresh.js<br/>
                • api/tg-monitor.js<br/>
                (Corrige erro de 13 funções no Vercel)
              </div>
            </div>
          </div>
        )}
...
export default function App() {
  ...
  useEffect(()=>{
    try {
      ...
      fetch("/api/ml-video?action=refresh", {
        method:"POST", headers:{"Content-Type":"application/json"},
