export default async function handler(req,res){
  ...
  try{
    const r=await fetch("https://api.groq.com/openai/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${K}`},body:JSON.stringify({model:"llama-3.3-70b-versatile",max_tokens:1000,messages:[{role:"system",content:"Você é ARIA, assistente de marketing de afiliados brasileiro. Responda sempre em português do Brasil."},{role:"user",content:prompt}]})});
